...
| **听觉补丁** | Whisper | 针对低置信度片段二次"精听" | **默认仅提供文本**，可选启用 WhisperX 强制对齐 |
| **逻辑胶水** | LLM | 错别字修正、歧义仲裁、翻译 | 最终文本裁决者 |

### 1.3 关键决策

1. **强制对齐可选**：WhisperX/MFA 对齐作为可选模块，需高精度时间轴时启用
2. **伪对齐算法**：Whisper/LLM 替换文本后，默认采用"线性时间平摊"策略
3. **前端虚拟滚动**：字级置信度数据量大，需实现 Virtual Scrolling
4. **流式输出**：各阶段完成即推送，实现实时字幕预览
...
| **预设1** | 智能补刀 | SV + Whisper Partial | 一般视频、日常Vlog |
...
export interface PresetConfig {
  ...
  enableWhisperXAlign: boolean;  // WhisperX 强制对齐（可选）
  // 校对层
...
export const ADVANCED_OPTIONS = {
  ...
  enhancement: {
    ...
    options: [
      ...
      { value: 'smart_patch', label: '智能补刀', desc: '低置信度自动 Whisper 补刀' },
...
| 修改项 | 文件 | 类型 | 优先级 |
|--------|------|------|--------|
| 熔断决策增加"仅文本补刀"选项 | `fuse_breaker.py` | 修改 | P0 |
...
| 修改项 | 文件 | 类型 | 优先级 |
|--------|------|------|--------|
| **WhisperX 强制对齐改为可选模块** | `transcription_service.py` | 修改 | P0 |
| Whisper 补刀仅取文本+伪对齐 | `transcription_service.py` | 修改 | P0 |
...
class TranscriptionPreset:
    ...
    enable_whisperx_align: bool = False            # WhisperX 强制对齐
...
class DemucsSettings:
    ...
    # ========== 新增：WhisperX 控制 ==========
    enable_whisperx: bool = False               # 是否启用 WhisperX 强制对齐
    whisperx_fallback_only: bool = True         # 仅在伪对齐不满足需求时使用
```
...
class JobState:
    ...
    segments_whisper_patched: int = 0            # Whisper 补刀完成数
    ...
    base = {
        ...
        "settings": {
            ...
            "preset": {
                ...
                "enable_whisperx_align": self.settings.preset.enable_whisperx_align,
...
def from_meta_dict(cls, data: dict) -> "JobState":
    ...
    preset = TranscriptionPreset(
        ...
        enable_whisperx_align=preset_data.get("enable_whisperx_align", False),
...
### 4.3 WhisperX 强制对齐（可选模块）

**文件**: `backend/app/services/whisperx_align_service.py`

> **重要变更**：WhisperX 强制对齐从必选改为可选模块
...
class WhisperXAlignService:
    ...
    def set_enabled(self, enabled: bool):
        ...
        logger.info(f"WhisperX 强制对齐: {'启用' if enabled else '禁用'}")
    ...
    def align_sentence(
        ...
        """
        if not self.enabled:
            logger.debug("WhisperX 未启用，跳过强制对齐")
            return sentence
        ...
        try:
            ...
            # 懒加载对齐模型
            if self._model is None or self._model_lang != language:
                logger.info(f"加载 WhisperX 对齐模型 (语言: {language})")
                self._model, self._metadata = whisperx.load_align_model(
                    language_code=language,
                    device="cuda"
                )
                self._model_lang = language
            ...
            result = whisperx.align(
                [{"text": sentence.text, "start": 0, "end": sentence.end - sentence.start}],
                self._model,
                self._metadata,
                audio_segment,
                device="cuda"
            ...
            if result and "word_segments" in result:
                ...
                for w in result["word_segments"]:
                    new_words.append(WordTimestamp(
                        ...
                        is_pseudo=False  # 真实对齐
                    ))
                sentence.words = new_words
                sentence.source = TextSource.WHISPERX_ALIGN
                logger.debug(f"WhisperX 对齐完成: {len(new_words)} 个字")

        except Exception as e:
            logger.warning(f"WhisperX 对齐失败，保持伪对齐: {e}")
...
_whisperx_align_instance = None


def get_whisperx_align_service() -> WhisperXAlignService:
    """获取 WhisperX 对齐服务单例"""
    global _whisperx_align_instance
    if _whisperx_align_instance is None:
        _whisperx_align_instance = WhisperXAlignService()
    return _whisperx_align_instance
```
...
class FuseAction(Enum):
    ...
    WHISPER_RETRY = "whisper_retry"             # Whisper 补刀（全量重试）
...
def decide_action(
    ...
    enable_whisperx: bool = False  # 新增：是否启用 WhisperX
) -> FuseDecision:
    """
    决策熔断动作（扩展版）
    ...
    4. 启用 WhisperX → WHISPERX_ALIGN（可选）
    5. 有 Whisper 备选 + 歧义 → LLM_ARBITRATE（新）
    """
    ...
    if confidence < self.upgrade_threshold:
        ...
        # 4.1 如果启用 WhisperX，先补刀后强制对齐
        if enable_whisperx:
            return FuseDecision(
                action=FuseAction.WHISPERX_ALIGN,
                reason=f"低置信度 {confidence:.2f}，Whisper 补刀 + WhisperX 对齐"
            )

        return FuseDecision(
            action=FuseAction.WHISPER_TEXT_ONLY,
            reason=f"低置信度 {confidence:.2f}，使用 Whisper 补刀（伪对齐）"
        )
...
async def _whisper_text_only_patch(
    ...
    """
    Whisper 仅文本补刀（时空解耦核心）

    关键原则：
    - 仅使用 Whisper 的文本输出
    - 默认使用伪对齐算法生成字级时间戳
    - 可选启用 WhisperX 强制对齐获得精准时间戳

    Args:
        sentence: 原始句子（SenseVoice 输出）
        audio_array: 音频数组
        previous_text: 前一句文本（用于 initial_prompt）
        enable_whisperx: 是否启用 WhisperX 强制对齐

    Returns:
        修正后的句子（时间戳不变或由 WhisperX 更新）
    """
    from .whisper_service import get_whisper_service
    from .pseudo_alignment import get_pseudo_alignment
    from .whisperx_align_service import get_whisperx_align_service
    ...
    # 6. 可选：WhisperX 强制对齐
    if enable_whisperx:
        whisperx_service = get_whisperx_align_service()
        whisperx_service.align_sentence(
            sentence=sentence,
    ...
    self.logger.info(
        f"Whisper 补刀完成: '{sentence.original_text}' -> '{sentence.text}'"
        f" [WhisperX: {'启用' if enable_whisperx else '禁用'}]"
    )
...
class ProcessPhase(Enum):
    ...
    WHISPER_PATCH = "whisper"     # Whisper 补刀
    WHISPERX_ALIGN = "whisperx"   # WhisperX 对齐（可选）
...
class SolutionConfig:
    ...
    # 高级选项
    confidence_threshold: float = 0.6             # 低于此值触发补刀
    enable_whisperx_align: bool = False           # 是否启用 WhisperX 强制对齐
...
class StreamingSubtitleManager:
    ...
    def on_whisper_patch_complete(
        ...
        if sentence_index >= len(self.sentences):
            logger.warning(f"Whisper 补刀索引越界: {sentence_index}")
            return
        ...
        logger.debug(f"Whisper 补刀 句子{sentence_index}: '{sent['original_text']}' -> '{new_text}'")
...
| **SV 置信度触发补刀** | 0.6 | 实测低于此值错误率显著上升 | 0.55-0.70 |
...
│     ↓                                                           │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ 8. WhisperX 强制对齐 [whisperx]（可选模块）                 ││
│  │    - 仅在启用时执行                                         ││
│  │    → SSE: progress.whisperx                                 ││
│  └─────────────────────────────────────────────────────────────┘│
│     ↓                                                           │
│  ═══════════════════════════════════════════════════════════════│
│  │               语义增强层（根据预设启用）                    ││
        ...
        if solution_config.enhancement != EnhancementMode.OFF:
            ...
            if low_confidence_sentences:
                progress.start_phase(
                    ...
                    message=f"Whisper 补刀中 (0/{len(low_confidence_sentences)})..."
                )
                ...
                for patch_idx, (sent_idx, sentence) in enumerate(low_confidence_sentences):
                    ...
                    progress.update_phase(
                        ...
                        message=f"Whisper 补刀中 ({patch_idx + 1}/{len(low_confidence_sentences)})..."
                    )
                    self._push_progress_update(job, progress)

                progress.complete_phase(ProcessPhase.WHISPER_PATCH)

        # === 阶段 8: WhisperX 强制对齐（可选模块） ===
        if solution_config.enable_whisperx_align:
            whisperx_service = get_whisperx_align_service()
            whisperx_service.set_enabled(True)

            progress.start_phase(
                ProcessPhase.WHISPERX_ALIGN,
                total_items=len(all_sentences),
                message="WhisperX 强制对齐中..."
            )
            ...
            for sent_idx, sentence in enumerate(all_sentences):
                ...
                progress.update_phase(ProcessPhase.WHISPERX_ALIGN, completed=sent_idx + 1)

            progress.complete_phase(ProcessPhase.WHISPERX_ALIGN)
...
- [ ] Whisper 补刀后时间戳仍由 SenseVoice 确定（或由 WhisperX 更新）
- [ ] WhisperX 强制对齐作为可选模块正常工作
- [ ] 伪对齐算法正确生成字级时间戳
- [ ] `source` 字段正确追踪文本来源
- [ ] 智能进度系统根据预设动态调整权重
...
- [ ] `subtitle.whisper_patch` 每句补刀完成后覆盖推送
