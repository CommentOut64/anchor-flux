...
> **核心变更摘要**：
> 本补充文档确立了 **“SenseVoice 为时间轴基准，Whisper/LLM 为内容修正者”** 的时空解耦架构。移除了对 WhisperX 强制对齐的硬性依赖，引入了模块化的处理流矩阵，并重点优化了 LLM 稀疏校对的上下文策略。
...
1.  **废弃强制对齐**：不再要求 Whisper 补刀后必须进行 WhisperX/MFA 对齐。
...
| **B2** | SV + Whisper Partial + **WhisperX** | 极高精度时间轴需求 | **高成本**。补刀后强制对齐，防止线性平摊导致的微小音画不同步。 |
