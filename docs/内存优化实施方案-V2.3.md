# 视频转录系统内存优化实施方案 V2.3

## 📋 方案概述

### 核心目标

通过"**双模式架构**"实现高性能转录与稳定性的平衡：
- ✅ **内存模式**（默认）：一次加载 + 内存切片，消除磁盘IO瓶颈
- ✅ **硬盘模式**（降级）：保留原有分段逻辑，应对内存不足场景
- ✅ **智能切换**：基于实时内存检测，自动无感降级
- ✅ **批次对齐**：实时SSE进度推送，前端进度条流畅更新

### 性能预期

| 优化项 | 硬盘模式（原） | 内存模式（新） | 收益 |
|--------|---------------|---------------|------|
| 磁盘IO次数（1hr视频） | ~120次写入+读取 | 2次大文件读取 | **减少98%** |
| 分段阶段耗时 | 10-30秒 | <2秒 | **5-15倍加速** |
| 对齐阶段耗时 | 重复加载audio.wav | 复用内存数组 | **2-3倍加速** |
| 内存占用（1hr视频） | ~200MB + 临时文件 | ~230MB纯内存 | 略增但可控 |

### 内存消耗估算公式
```
内存需求 (MB) = 音频时长(秒) × 16000 × 4 / 1024 / 1024
           ≈ 音频时长(分钟) × 3.66 MB
```

| 视频时长 | 估算内存需求 | 推荐模式 |
|---------|-------------|---------|
| 30分钟  | ~110 MB | 内存模式 |
| 1小时   | ~220 MB | 内存模式 |
| 2小时   | ~440 MB | 内存模式 |
| 3小时   | ~660 MB | 内存模式（8GB+系统） |
| 5小时+  | >1.1 GB | 硬盘模式（自动降级） |

---

## 🏗️ 双模式架构设计

### 架构总览
```
                    ┌─────────────────────────────────────┐
                    │         _run_pipeline 入口          │
                    └─────────────────┬───────────────────┘
                                      ↓
                    ┌─────────────────────────────────────┐
                    │      Phase 1: 提取音频 (FFmpeg)     │
                    │         → audio.wav                 │
                    └─────────────────┬───────────────────┘
                                      ↓
                    ┌─────────────────────────────────────┐
                    │      🆕 内存检测与模式决策          │
                    │   _decide_processing_mode()         │
                    └──────────┬──────────────┬───────────┘
                               ↓              ↓
              ┌────────────────────┐  ┌────────────────────┐
              │   内存模式 (默认)   │  │  硬盘模式 (降级)   │
              │  use_memory=True   │  │  use_memory=False  │
              └────────┬───────────┘  └────────┬───────────┘
                       ↓                       ↓
              ┌────────────────────┐  ┌────────────────────┐
              │  whisperx.load_    │  │  保留原audio.wav   │
              │  audio() 加载到    │  │  不加载到内存      │
              │  audio_array       │  │                    │
              └────────┬───────────┘  └────────┬───────────┘
                       ↓                       ↓
              ┌────────────────────┐  ┌────────────────────┐
              │ Phase 2: VAD分段   │  │ Phase 2: pydub分段 │
              │ _split_in_memory() │  │ _split_to_disk()   │
              │ 纯内存元数据       │  │ segment_N.wav文件  │
              └────────┬───────────┘  └────────┬───────────┘
                       ↓                       ↓
                       └───────────┬───────────┘
                                   ↓
              ┌─────────────────────────────────────────────┐
              │  🔒 强制刷新 checkpoint.json（分段元数据）  │
              │     确保断点续传的绝对一致性                │
              └─────────────────────┬───────────────────────┘
                                    ↓
              ┌────────────────────┐  ┌────────────────────┐
              │ Phase 3: 内存切片  │  │ Phase 3: 文件读取  │
              │ 转录 (Zero-copy)   │  │ 转录               │
              └────────┬───────────┘  └────────┬───────────┘
                       ↓                       ↓
                       └───────────┬───────────┘
                                   ↓
              ┌─────────────────────────────────────────────┐
              │     Phase 4: 批次对齐 + SSE实时进度推送     │
              │     _align_batched() + _push_align_progress │
              └─────────────────────┬───────────────────────┘
                                    ↓
              ┌─────────────────────────────────────────────┐
              │              Phase 5: 生成SRT               │
              └─────────────────────────────────────────────┘
```

---

## 🔧 Phase 1: 音频提取（保持不变）

**位置：** `transcription_service.py` → `_extract_audio()`

此阶段逻辑不变，继续使用FFmpeg提取音频为 `audio.wav`。

**测试点：**

- ✅ FFmpeg正常提取
- ✅ 输出文件存在性检查
- ✅ 超时处理（10分钟）

---

## 🔧 Phase 2: 智能模式决策与分段

### 2.1 内存检测与模式决策

**新增方法：** `_decide_processing_mode()`

```python
import psutil
from enum import Enum

class ProcessingMode(Enum):
    """处理模式枚举"""
    MEMORY = "memory"  # 内存模式（默认，高性能）
    DISK = "disk"      # 硬盘模式（降级，稳定性优先）


def _decide_processing_mode(self, audio_path: str, job: JobState) -> ProcessingMode:
    """
    智能决策处理模式（内存模式 vs 硬盘模式）

    决策逻辑：
    1. 估算音频内存需求
    2. 检测系统可用内存
    3. 预留安全余量（模型、转录中间变量等）
    4. 决定使用哪种模式

    Args:
        audio_path: 音频文件路径
        job: 任务状态对象

    Returns:
        ProcessingMode: 处理模式
    """
    # 获取音频时长（秒）
    audio_duration_sec = self._get_audio_duration(audio_path)

    # 估算音频内存需求 (16kHz, float32)
    # 公式: duration * 16000 * 4 bytes
    estimated_audio_mb = (audio_duration_sec * 16000 * 4) / (1024 * 1024)

    # 预留额外内存（模型加载、VAD处理、转录中间变量等）
    # 保守估计：音频内存的2倍 + 500MB基础开销
    total_estimated_mb = estimated_audio_mb * 2 + 500

    # 获取系统可用内存
    mem_info = psutil.virtual_memory()
    available_mb = mem_info.available / (1024 * 1024)
    total_mb = mem_info.total / (1024 * 1024)

    # 安全阈值：至少保留系统总内存的20%或2GB（取较大值）
    safety_reserve_mb = max(total_mb * 0.2, 2048)
    usable_mb = available_mb - safety_reserve_mb

    self.logger.info(f"📊 内存评估:")
    self.logger.info(f"   音频时长: {audio_duration_sec/60:.1f}分钟")
    self.logger.info(f"   预估需求: {total_estimated_mb:.0f}MB")
    self.logger.info(f"   可用内存: {available_mb:.0f}MB")
    self.logger.info(f"   安全余量: {safety_reserve_mb:.0f}MB")
    self.logger.info(f"   可用于处理: {usable_mb:.0f}MB")

    # 决策
    if usable_mb >= total_estimated_mb:
        self.logger.info("✅ 选择【内存模式】- 内存充足，使用高性能模式")
        job.message = "内存充足，使用高性能模式"
        return ProcessingMode.MEMORY
    else:
        self.logger.warning(f"⚠️ 选择【硬盘模式】- 内存不足（需要{total_estimated_mb:.0f}MB，可用{usable_mb:.0f}MB）")
        job.message = "内存受限，使用稳定模式"
        return ProcessingMode.DISK


def _get_audio_duration(self, audio_path: str) -> float:
    """
    获取音频时长（秒）

    Args:
        audio_path: 音频文件路径

    Returns:
        float: 音频时长（秒）
    """
    try:
        # 方法1: 使用pydub（精确但较慢）
        from pydub import AudioSegment
        audio = AudioSegment.from_wav(audio_path)
        return len(audio) / 1000.0
    except Exception as e:
        self.logger.warning(f"pydub获取时长失败，使用文件大小估算: {e}")
        # 方法2: 根据文件大小估算（16kHz, 16bit, mono ≈ 32KB/秒）
        import os
        file_size = os.path.getsize(audio_path)
        return file_size / 32000
```

### 2.2 安全加载音频到内存

**新增方法：** `_safe_load_audio()`

```python
def _safe_load_audio(self, audio_path: str, job: JobState) -> Optional[np.ndarray]:
    """
    安全加载音频到内存（带异常处理）

    Args:
        audio_path: 音频文件路径
        job: 任务状态对象

    Returns:
        np.ndarray: 音频数组，失败返回None

    Raises:
        RuntimeError: 音频加载失败时抛出，触发降级
    """
    try:
        self.logger.info(f"🔄 加载音频到内存: {audio_path}")
        audio_array = whisperx.load_audio(audio_path)

        # 验证加载结果
        if audio_array is None or len(audio_array) == 0:
            raise ValueError("音频数组为空")

        # 记录加载信息
        duration_sec = len(audio_array) / 16000
        memory_mb = audio_array.nbytes / (1024 * 1024)
        self.logger.info(f"✅ 音频加载成功:")
        self.logger.info(f"   时长: {duration_sec/60:.1f}分钟")
        self.logger.info(f"   内存占用: {memory_mb:.1f}MB")
        self.logger.info(f"   采样点数: {len(audio_array):,}")

        return audio_array

    except MemoryError as e:
        self.logger.error(f"❌ 内存不足，无法加载音频: {e}")
        job.message = "内存不足，自动切换到硬盘模式"
        raise RuntimeError(f"内存不足: {e}")

    except Exception as e:
        self.logger.error(f"❌ 音频加载失败: {e}")
        job.message = f"音频加载失败: {e}"
        raise RuntimeError(f"音频加载失败（可能文件损坏）: {e}")
```

### 2.3 VAD模型配置

#### VAD模型对比

| 特性 | Silero VAD（默认） | Pyannote VAD（可选） |
|------|-------------------|---------------------|
| 认证需求 | **无需认证** | 需要HuggingFace Token |
| 下载方式 | torch.hub自动下载 | 需要接受协议后下载 |
| 准确度 | 良好 | 更高 |
| 速度 | **更快** | 较慢 |
| 内存占用 | **更低** | GPU占用较高 |
| 推荐场景 | 默认使用 | 追求最高精度时启用 |

#### VAD配置枚举

```python
from enum import Enum

class VADMethod(Enum):
    """VAD模型选择"""
    SILERO = "silero"      # 默认，无需认证，速度快
    PYANNOTE = "pyannote"  # 可选，需要HF Token，精度更高
```

#### VAD配置数据结构

```python
from dataclasses import dataclass
from typing import Optional

@dataclass
class VADConfig:
    """VAD配置（用于设置界面）"""
    method: VADMethod = VADMethod.SILERO  # 默认使用Silero
    hf_token: Optional[str] = None         # Pyannote需要的HF Token
    onset: float = 0.5                     # 语音开始阈值
    offset: float = 0.363                  # 语音结束阈值
    chunk_size: int = 30                   # 最大段长（秒）

    def validate(self) -> bool:
        """验证配置有效性"""
        if self.method == VADMethod.PYANNOTE and not self.hf_token:
            return False  # Pyannote需要Token
        return True
```

#### 前端设置界面接口（预留）

**API端点：** `POST /api/settings/vad`

```typescript
// 前端请求体
interface VADSettingsRequest {
  method: "silero" | "pyannote";
  hf_token?: string;  // 仅当method="pyannote"时需要
  onset?: number;     // 可选，默认0.5
  offset?: number;    // 可选，默认0.363
}

// 前端逻辑（伪代码）
function onVADMethodChange(method: string) {
  if (method === "pyannote") {
    // 弹窗提示输入HuggingFace Token
    showDialog({
      title: "启用Pyannote VAD",
      message: "Pyannote VAD精度更高，但需要HuggingFace Token才能下载模型。",
      input: {
        label: "HuggingFace Token",
        placeholder: "hf_xxxxxxxxxxxx",
        type: "password"
      },
      links: [
        { text: "如何获取Token?", url: "https://huggingface.co/settings/tokens" }
      ],
      onConfirm: (token) => saveVADSettings({ method: "pyannote", hf_token: token }),
      onCancel: () => revertToSilero()
    });
  } else {
    // 直接保存Silero配置
    saveVADSettings({ method: "silero" });
  }
}
```

### 2.4 内存模式：VAD分段

**新增方法：** `_split_audio_in_memory()`

```python
def _split_audio_in_memory(
    self,
    audio_array: np.ndarray,
    sr: int = 16000,
    vad_config: Optional[VADConfig] = None
) -> List[Dict]:
    """
    内存VAD分段（不产生磁盘IO）

    默认使用Silero VAD（无需认证），可通过配置切换到Pyannote VAD。

    Args:
        audio_array: 完整音频数组 (np.ndarray, float32, 16kHz)
        sr: 采样率（默认16000Hz）
        vad_config: VAD配置（可选，默认使用Silero）

    Returns:
        List[Dict]: 分段元数据列表
        [
            {"index": 0, "start": 0.0, "end": 30.5, "mode": "memory"},
            {"index": 1, "start": 30.5, "end": 58.2, "mode": "memory"},
            ...
        ]
    """
    import torch

    # 使用默认配置
    if vad_config is None:
        vad_config = VADConfig()  # 默认Silero

    self.logger.info(f"🔍 开始内存VAD分段 (模型: {vad_config.method.value})...")

    try:
        # 根据配置选择VAD模型
        if vad_config.method == VADMethod.SILERO:
            segments = self._vad_silero(audio_array, sr, vad_config)
        else:
            segments = self._vad_pyannote(audio_array, sr, vad_config)

        self.logger.info(f"✅ VAD分段完成: {len(segments)}段 (模型: {vad_config.method.value})")
        return segments

    except Exception as e:
        self.logger.error(f"❌ VAD分段失败: {e}")
        # 降级到简易能量检测
        self.logger.warning("⚠️ 尝试降级到能量检测分段...")
        return self._energy_based_split(audio_array, sr, vad_config.chunk_size)


def _vad_silero(
    self,
    audio_array: np.ndarray,
    sr: int,
    vad_config: VADConfig
) -> List[Dict]:
    """
    Silero VAD分段（默认方案，无需认证）

    优点：
    - 无需HuggingFace Token
    - 通过torch.hub自动下载
    - 速度快，内存占用低
    """
    import torch

    self.logger.info("📦 加载Silero VAD模型...")

    # 从torch.hub加载Silero VAD
    model, utils = torch.hub.load(
        repo_or_dir='snakers4/silero-vad',
        model='silero_vad',
        force_reload=False,
        onnx=False
    )

    (get_speech_timestamps, _, read_audio, _, _) = utils

    # 转换为torch tensor
    audio_tensor = torch.from_numpy(audio_array)

    # 获取语音时间戳
    speech_timestamps = get_speech_timestamps(
        audio_tensor,
        model,
        sampling_rate=sr,
        threshold=vad_config.onset,  # 检测阈值
        min_speech_duration_ms=250,   # 最小语音段长度
        min_silence_duration_ms=100,  # 最小静音长度
    )

    # 合并分段（确保每段不超过chunk_size秒）
    segments_metadata = []
    current_start = None
    current_end = None

    for ts in speech_timestamps:
        start_sec = ts['start'] / sr
        end_sec = ts['end'] / sr

        if current_start is None:
            current_start = start_sec
            current_end = end_sec
        elif (end_sec - current_start) <= vad_config.chunk_size:
            # 可以合并
            current_end = end_sec
        else:
            # 保存当前段，开始新段
            segments_metadata.append({
                "index": len(segments_metadata),
                "start": current_start,
                "end": current_end,
                "mode": "memory"
            })
            current_start = start_sec
            current_end = end_sec

    # 保存最后一段
    if current_start is not None:
        segments_metadata.append({
            "index": len(segments_metadata),
            "start": current_start,
            "end": current_end,
            "mode": "memory"
        })

    return segments_metadata


def _vad_pyannote(
    self,
    audio_array: np.ndarray,
    sr: int,
    vad_config: VADConfig
) -> List[Dict]:
    """
    Pyannote VAD分段（高精度方案，需要HF Token）

    优点：
    - 精度更高
    - 支持更复杂的语音活动检测

    注意：
    - 需要HuggingFace Token
    - 首次使用需要接受模型使用协议
    """
    import torch
    from whisperx.vads import Pyannote

    if not vad_config.hf_token:
        raise ValueError("Pyannote VAD需要HuggingFace Token，请在设置中配置")

    self.logger.info("📦 加载Pyannote VAD模型（需要HF Token）...")

    # 初始化Pyannote VAD
    vad_model = Pyannote(
        vad_onset=vad_config.onset,
        vad_offset=vad_config.offset,
        use_auth_token=vad_config.hf_token,
        device=torch.device("cpu")  # 使用CPU避免占用GPU显存
    )

    # 准备输入
    audio_tensor = torch.from_numpy(audio_array).unsqueeze(0)

    # 执行VAD
    vad_segments = vad_model({
        "waveform": audio_tensor,
        "sample_rate": sr
    })

    # 合并分段
    merged = Pyannote.merge_chunks(
        vad_segments,
        chunk_size=vad_config.chunk_size,
        onset=vad_config.onset,
        offset=vad_config.offset
    )

    # 转换为标准格式
    segments_metadata = []
    for idx, seg in enumerate(merged):
        segments_metadata.append({
            "index": idx,
            "start": float(seg["start"]),
            "end": float(seg["end"]),
            "mode": "memory"
        })

    return segments_metadata


def _energy_based_split(
    self,
    audio_array: np.ndarray,
    sr: int,
    chunk_size: int = 30
) -> List[Dict]:
    """
    基于能量的简易分段（降级方案）

    当VAD模型加载失败时使用，按固定时长分段。
    """
    self.logger.warning("⚠️ 使用能量检测降级分段（固定时长）")

    total_duration = len(audio_array) / sr
    segments_metadata = []
    pos = 0.0

    while pos < total_duration:
        end = min(pos + chunk_size, total_duration)
        segments_metadata.append({
            "index": len(segments_metadata),
            "start": pos,
            "end": end,
            "mode": "memory"
        })
        pos = end

    return segments_metadata
```

### 2.5 硬盘模式：保留原有pydub分段

**整理现有方法：** `_split_audio_to_disk()`

```python
def _split_audio_to_disk(self, audio_path: str) -> List[Dict]:
    """
    硬盘分段模式（保留原有逻辑）

    使用pydub进行静音检测，生成segment_N.wav文件。
    适用于内存不足的场景。

    Args:
        audio_path: 音频文件路径

    Returns:
        List[Dict]: 分段信息列表
        [
            {"index": 0, "file": "segment_0.wav", "start": 0.0, "end": 30.0, "mode": "disk"},
            ...
        ]
    """
    from pydub import AudioSegment, silence

    self.logger.info("📀 开始硬盘分段（pydub静音检测）...")

    # 使用配置中的音频处理参数
    audio_config = config.get_audio_config()
    SEGMENT_LEN_MS = audio_config['segment_length_ms']
    SILENCE_SEARCH_MS = audio_config['silence_search_ms']
    MIN_SILENCE_LEN_MS = audio_config['min_silence_len_ms']
    SILENCE_THRESH_DBFS = audio_config['silence_threshold_dbfs']

    audio = AudioSegment.from_wav(audio_path)
    length = len(audio)
    segments = []
    pos = 0
    idx = 0

    while pos < length:
        end = min(pos + SEGMENT_LEN_MS, length)

        # 智能寻找静音点
        if end < length and (end - pos) > SILENCE_SEARCH_MS:
            search_start = max(pos, end - SILENCE_SEARCH_MS)
            search_chunk = audio[search_start:end]

            try:
                silences = silence.detect_silence(
                    search_chunk,
                    min_silence_len=MIN_SILENCE_LEN_MS,
                    silence_thresh=SILENCE_THRESH_DBFS
                )
                if silences:
                    silence_start = silences[0][0]
                    new_end = search_start + silence_start
                    if new_end - pos > MIN_SILENCE_LEN_MS:
                        end = new_end
            except Exception as e:
                self.logger.warning(f"静音检测失败: {e}")

        # 导出分段文件
        chunk = audio[pos:end]
        seg_file = os.path.join(os.path.dirname(audio_path), f'segment_{idx}.wav')
        chunk.export(seg_file, format='wav')

        segments.append({
            'index': idx,
            'file': seg_file,
            'start': pos / 1000.0,       # 转换为秒
            'end': end / 1000.0,         # 转换为秒
            'mode': 'disk'               # 标记模式
        })

        pos = end
        idx += 1

    self.logger.info(f"✅ 硬盘分段完成: {len(segments)}段 (生成segment文件)")
    return segments
```

### 2.6 分段后强制刷新Checkpoint

**关键要求：** VAD分段完成后，必须**立即**将分段元数据持久化到磁盘。

```python
def _flush_checkpoint_after_split(
    self,
    job_dir: Path,
    job: JobState,
    segments: List[Dict],
    processing_mode: ProcessingMode
):
    """
    分段完成后强制刷新checkpoint（确保断点续传一致性）

    ⚠️ 重要：这是断点续传的关键节点！
    只有分段元数据被持久化后，后续的转录索引才有意义。

    Args:
        job_dir: 任务目录
        job: 任务状态对象
        segments: 分段元数据列表
        processing_mode: 当前处理模式
    """
    checkpoint_data = {
        "job_id": job.job_id,
        "phase": "split_complete",  # 明确标记分段完成
        "processing_mode": processing_mode.value,  # 记录模式
        "total_segments": len(segments),
        "processed_indices": [],
        "segments": segments,
        "unaligned_results": [],
        "timestamp": time.time()  # 时间戳用于调试
    }

    # 强制同步写入（确保数据落盘）
    self._save_checkpoint(job_dir, checkpoint_data, job)

    # 验证写入成功
    saved_checkpoint = self._load_checkpoint(job_dir)
    if saved_checkpoint is None or saved_checkpoint.get('phase') != 'split_complete':
        raise RuntimeError("❌ checkpoint写入验证失败，无法继续")

    self.logger.info(f"💾 ✅ 分段checkpoint已强制刷新并验证 (模式: {processing_mode.value})")
```

---

## 🔧 Phase 3: 转录处理

### 3.1 内存模式：切片转录

**新增方法：** `_transcribe_segment_in_memory()`

```python
def _transcribe_segment_in_memory(
    self,
    audio_array: np.ndarray,
    seg_meta: Dict,
    model,
    job: JobState
) -> Optional[Dict]:
    """
    从内存切片转录（Zero-copy，高性能）

    Args:
        audio_array: 完整音频数组
        seg_meta: 分段元数据 {"index": 0, "start": 0.0, "end": 30.5, "mode": "memory"}
        model: Whisper模型
        job: 任务状态

    Returns:
        Dict: 未对齐的转录结果
    """
    sr = 16000
    start_sample = int(seg_meta['start'] * sr)
    end_sample = int(seg_meta['end'] * sr)

    # Zero-copy切片（numpy view，不复制数据）
    audio_slice = audio_array[start_sample:end_sample]

    try:
        # Whisper转录
        rs = model.transcribe(
            audio_slice,
            batch_size=job.settings.batch_size,
            verbose=False,
            language=job.language
        )

        if not rs or 'segments' not in rs:
            return None

        # 检测语言
        if not job.language and 'language' in rs:
            job.language = rs['language']
            self.logger.info(f"🌐 检测到语言: {job.language}")

        # 时间偏移校正
        start_offset = seg_meta['start']
        adjusted_segments = []

        for idx, s in enumerate(rs['segments']):
            adjusted_segments.append({
                'id': idx,
                'start': s.get('start', 0) + start_offset,
                'end': s.get('end', 0) + start_offset,
                'text': s.get('text', '').strip()
            })

        return {
            'segment_index': seg_meta['index'],
            'language': rs.get('language', job.language),
            'segments': adjusted_segments
        }

    finally:
        del audio_slice
        gc.collect()
```

### 3.2 硬盘模式：文件转录（保留原逻辑）

**整理现有方法：** `_transcribe_segment_from_disk()`

```python
def _transcribe_segment_from_disk(
    self,
    seg: Dict,
    model,
    job: JobState
) -> Optional[Dict]:
    """
    从文件加载转录（硬盘模式）

    Args:
        seg: 分段信息 {"file": "segment_0.wav", "start": 0.0, "end": 30.0, "mode": "disk"}
        model: Whisper模型
        job: 任务状态

    Returns:
        Dict: 未对齐的转录结果
    """
    audio = whisperx.load_audio(seg['file'])

    try:
        rs = model.transcribe(
            audio,
            batch_size=job.settings.batch_size,
            verbose=False,
            language=job.language
        )

        if not rs or 'segments' not in rs:
            return None

        if not job.language and 'language' in rs:
            job.language = rs['language']
            self.logger.info(f"🌐 检测到语言: {job.language}")

        start_offset = seg['start']
        adjusted_segments = []

        for idx, s in enumerate(rs['segments']):
            adjusted_segments.append({
                'id': idx,
                'start': s.get('start', 0) + start_offset,
                'end': s.get('end', 0) + start_offset,
                'text': s.get('text', '').strip()
            })

        return {
            'segment_index': seg['index'],
            'language': rs.get('language', job.language),
            'segments': adjusted_segments
        }

    finally:
        del audio
        gc.collect()
```

### 3.3 统一转录入口

```python
def _transcribe_segment(
    self,
    seg_meta: Dict,
    model,
    job: JobState,
    audio_array: Optional[np.ndarray] = None
) -> Optional[Dict]:
    """
    统一转录入口（根据模式自动选择）

    Args:
        seg_meta: 分段元数据
        model: Whisper模型
        job: 任务状态
        audio_array: 音频数组（内存模式时必须提供）

    Returns:
        Dict: 未对齐的转录结果
    """
    mode = seg_meta.get('mode', 'disk')

    if mode == 'memory':
        if audio_array is None:
            raise ValueError("内存模式需要提供audio_array")
        return self._transcribe_segment_in_memory(audio_array, seg_meta, model, job)
    else:
        return self._transcribe_segment_from_disk(seg_meta, model, job)
```

### 3.4 转录过程中的内存监控

```python
def _check_memory_during_transcription(self, job: JobState) -> bool:
    """
    转录过程中检查内存状态

    如果内存严重不足，暂停任务并警告用户

    Returns:
        bool: True=继续处理，False=需要暂停
    """
    mem_info = psutil.virtual_memory()
    available_mb = mem_info.available / (1024 * 1024)
    percent_used = mem_info.percent

    # 危险阈值：可用内存<500MB 或 使用率>95%
    if available_mb < 500 or percent_used > 95:
        self.logger.error(f"🚨 内存严重不足！可用: {available_mb:.0f}MB, 使用率: {percent_used}%")
        job.status = 'paused'
        job.message = f"⚠️ 内存不足暂停（可用{available_mb:.0f}MB），请关闭其他程序后继续"
        job.paused = True

        # 推送警告SSE
        self._push_sse_signal(job, "memory_warning",
            f"内存严重不足（可用{available_mb:.0f}MB），任务已暂停")

        return False

    # 警告阈值：可用内存<1GB 或 使用率>90%
    if available_mb < 1024 or percent_used > 90:
        self.logger.warning(f"⚠️ 内存紧张: 可用{available_mb:.0f}MB, 使用率{percent_used}%")
        # 不暂停，但记录警告

    return True
```

---

## 🔧 Phase 4: 批次对齐 + SSE进度推送（必要项）

### 4.1 批次对齐实现

**新增方法：** `_align_all_results_batched()`

```python
def _align_all_results_batched(
    self,
    unaligned_results: List[Dict],
    job: JobState,
    audio_source: Union[np.ndarray, str],  # 内存数组或文件路径
    processing_mode: ProcessingMode
) -> List[Dict]:
    """
    分批对齐（必要项）- 支持实时SSE进度推送

    批次对齐的优势：
    1. 避免一次性对齐所有内容导致的长时间卡顿
    2. 支持前端进度条实时更新
    3. 内存使用更可控

    Args:
        unaligned_results: 所有未对齐的转录结果
        job: 任务状态对象
        audio_source: 音频来源（内存模式传数组，硬盘模式传路径）
        processing_mode: 当前处理模式

    Returns:
        List[Dict]: 对齐后的结果
    """
    self.logger.info(f"🔧 开始批次对齐: {len(unaligned_results)}个分段")

    # 1. 合并所有segments
    all_segments = []
    for result in unaligned_results:
        all_segments.extend(result['segments'])

    if not all_segments:
        self.logger.warning("没有可对齐的内容")
        return []

    # 2. 加载音频（根据模式）
    if processing_mode == ProcessingMode.MEMORY:
        audio_array = audio_source  # 直接使用内存数组
        self.logger.info("✅ 对齐阶段复用内存中的音频数组")
    else:
        # 硬盘模式：需要加载完整音频
        self.logger.info("📀 对齐阶段加载完整音频...")
        audio_array = whisperx.load_audio(audio_source)

    # 3. 获取对齐模型
    lang = job.language or unaligned_results[0].get('language', 'zh')
    align_model, metadata = self._get_align_model(lang, job.settings.device, job)

    # 4. 分批对齐
    BATCH_SIZE = 50  # 每批50条segment
    total_segments = len(all_segments)
    total_batches = math.ceil(total_segments / BATCH_SIZE)
    aligned_segments = []

    self.logger.info(f"📊 对齐配置: 总{total_segments}条, 每批{BATCH_SIZE}条, 共{total_batches}批")

    for batch_idx in range(total_batches):
        start_idx = batch_idx * BATCH_SIZE
        end_idx = min(start_idx + BATCH_SIZE, total_segments)
        batch = all_segments[start_idx:end_idx]

        # 计算进度
        progress = batch_idx / total_batches

        # 更新任务进度
        self._update_progress(
            job,
            'align',
            progress,
            f'对齐中 {batch_idx + 1}/{total_batches}批'
        )

        # 🆕 推送对齐进度SSE（专用事件）
        self._push_sse_align_progress(
            job,
            batch_idx + 1,
            total_batches,
            len(aligned_segments),
            total_segments
        )

        # 执行对齐
        try:
            aligned_batch = whisperx.align(
                batch,
                align_model,
                metadata,
                audio_array,
                job.settings.device
            )
            aligned_segments.extend(aligned_batch.get('segments', []))
            self.logger.debug(f"✅ 批次 {batch_idx + 1}/{total_batches} 完成")

        except Exception as e:
            self.logger.error(f"❌ 批次 {batch_idx + 1} 对齐失败: {e}")
            # 继续处理其他批次，不中断整体流程
            continue

    # 5. 完成
    self._update_progress(job, 'align', 1, '对齐完成')
    self._push_sse_align_progress(job, total_batches, total_batches, total_segments, total_segments)

    self.logger.info(f"✅ 批次对齐完成: {len(aligned_segments)}条")

    # 如果是硬盘模式，释放加载的音频
    if processing_mode == ProcessingMode.DISK:
        del audio_array
        gc.collect()

    return [{
        'segments': aligned_segments,
        'word_segments': []
    }]
```

### 4.2 对齐进度SSE推送接口

**新增方法：** `_push_sse_align_progress()`

```python
def _push_sse_align_progress(
    self,
    job: JobState,
    current_batch: int,
    total_batches: int,
    aligned_count: int,
    total_count: int
):
    """
    推送对齐进度SSE事件（前端进度条实时更新）

    事件类型: "align_progress"

    Args:
        job: 任务状态对象
        current_batch: 当前批次号（1-based）
        total_batches: 总批次数
        aligned_count: 已对齐的segment数量
        total_count: 总segment数量
    """
    try:
        from services.sse_service import get_sse_manager
        sse_manager = get_sse_manager()

        channel_id = f"job:{job.job_id}"

        # 计算百分比
        batch_progress = (current_batch / total_batches) * 100 if total_batches > 0 else 0
        segment_progress = (aligned_count / total_count) * 100 if total_count > 0 else 0

        sse_manager.broadcast_sync(
            channel_id,
            "align_progress",  # 🆕 专用事件类型
            {
                "job_id": job.job_id,
                "phase": "align",
                "batch": {
                    "current": current_batch,
                    "total": total_batches,
                    "progress": round(batch_progress, 2)
                },
                "segments": {
                    "aligned": aligned_count,
                    "total": total_count,
                    "progress": round(segment_progress, 2)
                },
                "message": f"对齐中 {current_batch}/{total_batches}批 ({aligned_count}/{total_count}条)"
            }
        )

    except Exception as e:
        self.logger.debug(f"SSE对齐进度推送失败（非致命）: {e}")
```

### 4.3 前端SSE事件接口定义

**新增SSE事件：** `align_progress`

```typescript
// 前端TypeScript接口定义

interface AlignProgressEvent {
  job_id: string;
  phase: "align";
  batch: {
    current: number;   // 当前批次（1-based）
    total: number;     // 总批次数
    progress: number;  // 批次进度百分比 (0-100)
  };
  segments: {
    aligned: number;   // 已对齐数量
    total: number;     // 总数量
    progress: number;  // 对齐进度百分比 (0-100)
  };
  message: string;     // 用户友好的消息
}

// 前端监听示例
eventSource.addEventListener('align_progress', (event) => {
  const data: AlignProgressEvent = JSON.parse(event.data);
  // 更新对齐进度条
  updateAlignProgress(data.segments.progress);
  // 更新状态消息
  setStatusMessage(data.message);
});
```

---

## 🔧 Phase 5: 生成SRT（保持不变）

此阶段逻辑不变，继续使用现有的 `_generate_srt()` 方法。

---

## 📝 检查点数据结构

### 新版checkpoint.json结构

```json
{
  "job_id": "abc123",
  "phase": "transcribe",
  "processing_mode": "memory",
  "total_segments": 60,
  "processed_indices": [0, 1, 2, 3],
  "segments": [
    {
      "index": 0,
      "start": 0.0,
      "end": 28.5,
      "mode": "memory"
    },
    {
      "index": 1,
      "start": 28.5,
      "end": 55.2,
      "mode": "memory"
    }
  ],
  "unaligned_results": [
    {
      "segment_index": 0,
      "language": "zh",
      "segments": [...]
    }
  ],
  "timestamp": 1700000000.123,
  "original_settings": {
    "model": "medium",
    "device": "cuda",
    "word_timestamps": false,
    "compute_type": "float16",
    "batch_size": 16
  }
}
```

**字段说明：**
- `processing_mode`: 记录处理模式（memory/disk），恢复时使用相同模式
- `segments[].mode`: 每个分段的模式标记
- `timestamp`: 最后更新时间戳，用于调试

---

## 🚀 分阶段开发计划

### 阶段划分原则

- 每个阶段独立可测试
- 阶段之间有明确的验收标准
- 出问题可快速定位到具体阶段

---

### 🔹 阶段A：基础设施（1-2小时）

**目标：** 添加内存检测和模式决策基础能力

**开发内容：**

1. 添加 `ProcessingMode` 枚举
2. 实现 `_get_audio_duration()` 方法
3. 实现 `_decide_processing_mode()` 方法
4. 添加 `psutil` 依赖

**验收标准：**
```python
# 测试代码
mode = service._decide_processing_mode("/path/to/audio.wav", job)
print(f"决策结果: {mode}")
# 应输出: ProcessingMode.MEMORY 或 ProcessingMode.DISK
```

**测试点：**
- [x] 短视频（10分钟）→ 返回 MEMORY
- [x] 模拟低内存 → 返回 DISK
- [x] 日志输出内存评估信息

---

### 🔹 阶段B：安全音频加载（30分钟）

**目标：** 实现带异常处理的音频加载

**开发内容：**

1. 实现 `_safe_load_audio()` 方法
2. 添加加载失败的异常处理
3. 添加加载成功的日志记录

**验收标准：**
```python
# 正常加载
audio_array = service._safe_load_audio("/path/to/valid.wav", job)
assert audio_array is not None
assert len(audio_array) > 0

# 异常处理（损坏文件）
try:
    service._safe_load_audio("/path/to/corrupted.wav", job)
except RuntimeError as e:
    print(f"正确捕获异常: {e}")
```

**测试点：**
- [ ] 正常WAV文件加载成功
- [ ] 损坏文件抛出RuntimeError
- [ ] 空文件抛出异常

---

### 🔹 阶段C：内存VAD分段（1小时）

**目标：** 实现内存模式的VAD分段（默认Silero，预留Pyannote接口）

**开发内容：**
1. 添加 `VADMethod` 枚举和 `VADConfig` 数据类
2. 实现 `_split_audio_in_memory()` 方法（统一入口）
3. 实现 `_vad_silero()` 方法（默认，无需认证）
4. 实现 `_vad_pyannote()` 方法（预留，需HF Token）
5. 实现 `_energy_based_split()` 降级方法

**验收标准：**
```python
# 加载音频
audio_array = whisperx.load_audio("/path/to/audio.wav")

# 默认使用Silero VAD
segments = service._split_audio_in_memory(audio_array)

# 验证
assert len(segments) > 0
assert all('index' in s and 'start' in s and 'end' in s for s in segments)
assert all(s['mode'] == 'memory' for s in segments)
print(f"分段数: {len(segments)}")
```

**测试点：**
- [ ] Silero VAD默认加载成功
- [ ] 分段数量合理（10分钟视频约20-40段）
- [ ] 时间戳连续无重叠
- [ ] 每段长度<=30秒
- [ ] VAD失败时正确降级到能量检测

---

### 🔹 阶段D：硬盘分段整理（30分钟）

**目标：** 整理现有硬盘分段逻辑，统一接口

**开发内容：**
1. 将现有 `_split_audio()` 重命名为 `_split_audio_to_disk()`
2. 统一返回格式（添加 `index`, `mode` 字段）
3. 保持功能不变

**验收标准：**
```python
segments = service._split_audio_to_disk("/path/to/audio.wav")

assert len(segments) > 0
assert all('index' in s and 'file' in s and 'mode' in s for s in segments)
assert all(s['mode'] == 'disk' for s in segments)
assert all(os.path.exists(s['file']) for s in segments)
```

**测试点：**
- [ ] 生成segment_N.wav文件
- [ ] 返回格式与内存模式一致
- [ ] 原有功能正常

---

### 🔹 阶段E：分段checkpoint强制刷新（30分钟）

**目标：** 确保分段完成后立即持久化

**开发内容：**
1. 实现 `_flush_checkpoint_after_split()` 方法
2. 添加写入验证逻辑
3. 在分段完成后调用

**验收标准：**
```python
# 分段后
segments = service._split_audio_in_memory(audio_array)

# 强制刷新
service._flush_checkpoint_after_split(job_dir, job, segments, ProcessingMode.MEMORY)

# 验证
checkpoint = service._load_checkpoint(job_dir)
assert checkpoint['phase'] == 'split_complete'
assert checkpoint['processing_mode'] == 'memory'
assert len(checkpoint['segments']) == len(segments)
```

**测试点：**
- [ ] checkpoint文件存在
- [ ] phase正确标记为split_complete
- [ ] segments数据完整

---

### 🔹 阶段F：双模式转录（1小时）

**目标：** 实现统一的转录入口，支持双模式

**开发内容：**
1. 实现 `_transcribe_segment_in_memory()` 方法
2. 整理 `_transcribe_segment_from_disk()` 方法
3. 实现 `_transcribe_segment()` 统一入口
4. 添加内存监控 `_check_memory_during_transcription()`

**验收标准：**
```python
# 内存模式
result = service._transcribe_segment(
    seg_meta, model, job, audio_array=audio_array
)
assert result['segment_index'] == seg_meta['index']

# 硬盘模式
result = service._transcribe_segment(
    seg_meta_disk, model, job, audio_array=None
)
assert result['segment_index'] == seg_meta_disk['index']
```

**测试点：**
- [ ] 内存模式转录正常
- [ ] 硬盘模式转录正常
- [ ] 时间偏移校正正确
- [ ] 内存监控触发警告

---

### 🔹 阶段G：批次对齐 + SSE进度（1-2小时）

**目标：** 实现必要的批次对齐和SSE进度推送

**开发内容：**
1. 实现 `_align_all_results_batched()` 方法
2. 实现 `_push_sse_align_progress()` 方法
3. 更新进度权重配置
4. 测试SSE事件

**验收标准：**

```python
# 批次对齐
aligned = service._align_all_results_batched(
    unaligned_results, job, audio_array, ProcessingMode.MEMORY
)

assert len(aligned) > 0
assert 'segments' in aligned[0]

# SSE事件验证（需要前端配合）
# 应收到多个 align_progress 事件
```

**测试点：**
- [ ] 批次数量正确
- [ ] 进度从0%到100%
- [ ] SSE事件正常推送
- [ ] 前端进度条更新

---

### 🔹 阶段H：主流程集成（1-2小时）

**目标：** 将所有组件集成到 `_run_pipeline()`

**开发内容：**
1. 重构 `_run_pipeline()` 主流程
2. 集成模式决策
3. 集成智能降级
4. 端到端测试

**验收标准：**

```bash
# 完整转录测试
# 上传视频 → 自动选择模式 → 分段 → 转录 → 对齐 → 生成SRT
# 观察：
# 1. 日志显示选择的模式
# 2. 对齐进度SSE正常
# 3. SRT文件正确生成
```

**测试点：**
- [ ] 短视频（内存模式）完整流程
- [ ] 长视频（硬盘模式）完整流程
- [ ] 断点续传正常
- [ ] 内存不足自动暂停

---

## 📊 进度权重配置更新

**位置：** `core/config.py`

```python
# 更新后的进度权重
self.PHASE_WEIGHTS = {
    "extract": 5,      # 音频提取占5%
    "split": 5,        # 音频分段占5%
    "transcribe": 60,  # 转录处理占60%（降低，因对齐权重增加）
    "align": 20,       # 对齐处理占20%（增加，因为是批次对齐）
    "srt": 10          # SRT生成占10%
}
```

---

## ⚠️ 风险控制

### 内存不足处理流程
```
检测到内存紧张（可用<1GB）
        ↓
    记录警告日志
        ↓
继续处理（尝试完成当前段）
        ↓
检测到内存严重不足（可用<500MB）
        ↓
    立即暂停任务
        ↓
  推送SSE警告事件
        ↓
 更新job状态为paused
        ↓
  用户看到明确警告
        ↓
用户关闭其他程序后恢复
```

### VAD模型失败降级
```python
def _split_audio_in_memory(self, audio_array, sr=16000):
    try:
        # 尝试使用VAD模型
        return self._vad_split(audio_array, sr)
    except Exception as e:
        self.logger.warning(f"⚠️ VAD模型失败，使用能量检测降级: {e}")
        # 降级方案：基于能量的简易分段
        return self._energy_based_split(audio_array, sr)
```

---

## ✅ 开发检查清单

### 阶段A：基础设施

- [ ] 添加 ProcessingMode 枚举
- [ ] 实现 _get_audio_duration()
- [ ] 实现 _decide_processing_mode()
- [ ] 测试通过

### 阶段B：安全音频加载
- [ ] 实现 _safe_load_audio()
- [ ] 异常处理完整
- [ ] 测试通过

### 阶段C：内存VAD分段
- [ ] 添加 VADMethod 枚举和 VADConfig 数据类
- [ ] 实现 _split_audio_in_memory() 统一入口
- [ ] 实现 _vad_silero() 默认方法
- [ ] 实现 _vad_pyannote() 预留方法
- [ ] 实现 _energy_based_split() 降级方法
- [ ] 测试Silero VAD加载成功
- [ ] 测试通过

### 阶段D：硬盘分段整理
- [ ] 重命名为 _split_audio_to_disk()
- [ ] 统一返回格式
- [ ] 测试通过

### 阶段E：checkpoint强制刷新
- [ ] 实现 _flush_checkpoint_after_split()
- [ ] 添加写入验证
- [ ] 测试通过

### 阶段F：双模式转录
- [ ] 实现 _transcribe_segment_in_memory()
- [ ] 整理 _transcribe_segment_from_disk()
- [ ] 实现统一入口
- [ ] 添加内存监控
- [ ] 测试通过

### 阶段G：批次对齐 + SSE
- [ ] 实现 _align_all_results_batched()
- [ ] 实现 _push_sse_align_progress()
- [ ] 前端接收事件正常
- [ ] 测试通过

### 阶段H：主流程集成

- [ ] 重构 _run_pipeline()
- [ ] 端到端测试通过
- [ ] 断点续传测试
- [ ] 内存不足测试

---

## 📚 依赖更新

### requirements.txt 新增
```txt
psutil>=5.9.0    # 内存监控
```

---

**文档版本：** V2.3
**最后更新：** 2025-11-23
**状态：** 待实施
**VAD默认模式：** Silero（无需认证）
