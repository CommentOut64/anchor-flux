# 任务监控器完整开发文档

> 基于单任务运行机制的任务监控系统重构方案
>
> **核心约束：系统底层设计只允许一个任务运行**

---

## 📋 目录

1. [系统设计原则](#1-系统设计原则)
2. [架构设计](#2-架构设计)
3. [数据结构设计](#3-数据结构设计)
4. [组件设计](#4-组件设计)
5. [技术实现方案](#5-技术实现方案)
6. [代码修改清单](#6-代码修改清单)
7. [实施步骤](#7-实施步骤)
8. [验收标准](#8-验收标准)

---

## 1. 系统设计原则

### 1.1 核心约束

**系统底层设计只允许一个任务运行**

这意味着：
- ✅ Processing 区域永远只有 0-1 个任务
- ✅ 不需要考虑多任务并行处理的界面逻辑
- ✅ 任务完成后，下一个排队任务自动开始
- ✅ 简化状态管理逻辑

### 1.2 设计理念

1. **渐进式披露 (Progressive Disclosure)**
   - Processing 任务常驻顶部，不可折叠
   - Queued 任务默认展开（用户需频繁调整优先级）
   - Failed/Paused/Finished 默认折叠（次要信息）

2. **关注点分离 (Separation of Concerns)**
   - 活跃任务（Processing/Queued）：强调实时性与操控
   - 历史任务（Finished）：强调归档与查阅，限制显示数量

3. **即时反馈 (Immediate Feedback)**
   - 状态变更伴随明确的视觉反馈
   - 使用 CSS + Vue TransitionGroup 实现流畅动画
   - 任务完成时显示短暂高亮

4. **性能优先**
   - 历史记录只渲染最近 20 条
   - 使用 CSS Grid 折叠（不引入虚拟滚动）
   - 避免复杂的 JS 动画编排

---

## 2. 架构设计

### 2.1 整体布局

```
┌─────────────────────────────────────┐
│ Header (Fixed)                      │  ← Processing 任务区域
│ - 最多显示 1 个 Processing 任务      │     (固定在顶部，max-height: 300px)
│ - 无任务时显示空状态                 │
└─────────────────────────────────────┘
┌─────────────────────────────────────┐
│ Body (Scrollable)                   │  ← 任务分组区域
│                                     │     (占据剩余空间，可滚动)
│ ┌─ SSE 断开警告 (动态显示) ────────┐│
│ │ ⚠️ 实时连接已断开，正在重连...    ││
│ └───────────────────────────────────┘│
│                                     │
│ ┌─ 失败任务 (默认折叠) ────────────┐│
│ │ 🔴 失败 (3)              [∧]    ││
│ └───────────────────────────────────┘│
│                                     │
│ ┌─ 排队中 (默认展开) ──────────────┐│
│ │ 🔵 排队中 (5)            [∨]    ││
│ │ ┌─────────────────────────────┐ ││
│ │ │ ⋮⋮ 任务 A  [50%]  [⏸][×]  │ ││  ← 可拖动排序
│ │ │ ⋮⋮ 任务 B  [0%]   [⏸][×]  │ ││
│ │ └─────────────────────────────┘ ││
│ └───────────────────────────────────┘│
│                                     │
│ ┌─ 已暂停 (默认折叠) ──────────────┐│
│ │ 🟡 已暂停 (2)            [∧]    ││
│ └───────────────────────────────────┘│
│                                     │
│ ┌─ 最近完成 (默认折叠) ────────────┐│
│ │ 🟢 最近完成 (20)         [∧]    ││
│ │ 查看全部历史记录 >              ││
│ └───────────────────────────────────┘│
│                                     │
└─────────────────────────────────────┘
┌─────────────────────────────────────┐
│ Footer (渐变遮罩)                   │  ← 视觉提示
│ ████████████████░░░░░░░░░░          │     (pointer-events: none)
└─────────────────────────────────────┘
```

### 2.2 状态流转图

```
[上传文件]
    ↓
[created] ──────→ [queued] ──────→ [processing] ──────→ [finished]
                     ↓                  ↓                    ↑
                     │                  │                    │
                     ↓                  ↓                    │
                  [paused] ←────────→ [failed] ─────────────┘
                     ↓
                     ↓
                  [canceled] (静默删除)
```

---

## 3. 数据结构设计

### 3.1 核心设计思想

**采用 Map（ID 索引）+ Array（队列顺序）双索引模式**

- `tasksMap`: 快速查找任务（O(1)）
- `queueOrder`: 维护队列顺序（单一事实来源）

### 3.2 Store 结构

```typescript
// stores/unifiedTaskStore.js

interface Task {
  job_id: string              // 任务 ID
  filename: string            // 文件名
  title?: string              // 用户自定义标题
  status: TaskStatus          // 任务状态
  progress: number            // 进度 0-100
  phase?: string              // 当前阶段 (extract/split/transcribe/align...)
  phase_percent?: number      // 阶段内进度
  message?: string            // 状态消息
  created_at: number          // 创建时间
  updated_at: number          // 更新时间
  completed_at?: number       // 完成时间
  paused_at?: number          // 暂停时间
  failed_at?: number          // 失败时间
  sseConnected?: boolean      // SSE 连接状态（前端字段）
  lastError?: string          // 最后错误（前端字段）
  isNewlyFinished?: boolean   // 刚完成标记（前端字段，用于高亮）
}

type TaskStatus = 'created' | 'queued' | 'processing' | 'paused' | 'finished' | 'failed' | 'canceled'

interface TaskStore {
  // 核心数据
  tasksMap: Map<string, Task>  // ID -> Task 映射
  queueOrder: string[]         // 队列顺序（仅包含 queued 状态的任务 ID）

  // SSE 状态
  sseConnected: boolean        // 全局 SSE 连接状态
  lastHeartbeat: number        // 最后心跳时间

  // 计算属性（Getters）
  processingTask: Task | null         // 当前处理中的任务（最多 1 个）
  queuedTasks: Task[]                 // 排队任务（按 queueOrder 排序）
  failedTasks: Task[]                 // 失败任务（按失败时间倒序）
  pausedTasks: Task[]                 // 暂停任务（按暂停时间倒序）
  recentFinishedTasks: Task[]         // 最近完成的任务（最多 20 条）

  // Actions
  addTask(task: Task): void
  updateTask(jobId: string, updates: Partial<Task>): void
  updateTaskProgress(jobId: string, progress: number, extraData?: object): void
  deleteTask(jobId: string): void
  reorderQueue(newOrder: string[]): Promise<void>
  checkSSEConnection(): void
  syncTasksFromBackend(): Promise<void>
}
```

### 3.3 关键逻辑

```javascript
// 1. Processing 任务（单例）
const processingTask = computed(() => {
  const tasks = Array.from(tasksMap.values()).filter(t => t.status === 'processing')
  return tasks.length > 0 ? tasks[0] : null  // 最多返回 1 个
})

// 2. 队列任务（严格按后端顺序）
const queuedTasks = computed(() => {
  return queueOrder.value
    .map(id => tasksMap.value.get(id))
    .filter(t => t && t.status === 'queued')
})

// 3. 历史任务（限制数量）
const recentFinishedTasks = computed(() =>
  Array.from(tasksMap.values())
    .filter(t => t.status === 'finished')
    .sort((a, b) => b.completed_at - a.completed_at)
    .slice(0, 20)  // 只取前 20 条
)

// 4. SSE 心跳检测
function checkSSEConnection() {
  if (Date.now() - lastHeartbeat.value > 30000) {  // 30 秒无心跳
    sseConnected.value = false
    console.warn('[TaskStore] SSE 连接超时')
  }
}

// 定期检测（在组件 mounted 时启动）
setInterval(checkSSEConnection, 5000)
```

---

## 4. 组件设计

### 4.1 组件树

```
TaskMonitor
├── ProcessingArea (固定区域)
│   └── TaskCard (最多 1 个)
│
├── ScrollableBody (滚动区域)
│   ├── SSEAlert (条件渲染)
│   ├── TaskGroup (失败任务)
│   │   └── TaskCard (多个)
│   ├── TaskGroup (排队中) **可拖动**
│   │   └── Draggable
│   │       └── TaskCard (多个)
│   ├── TaskGroup (已暂停)
│   │   └── TaskCard (多个)
│   └── TaskGroup (最近完成)
│       ├── TaskCard (最多 20 个)
│       └── ViewMoreButton
│
└── GradientMask (底部遮罩)
```

### 4.2 核心组件规格

#### 4.2.1 TaskMonitor (主容器)

```vue
<template>
  <div class="task-monitor">
    <!-- 固定区：Processing 任务 -->
    <div class="monitor-header">
      <TransitionGroup name="list">
        <TaskCard
          v-if="processingTask"
          :key="processingTask.job_id"
          :task="processingTask"
          variant="processing"
        />
      </TransitionGroup>

      <!-- 无任务时的空状态 -->
      <div v-if="!processingTask && !hasAnyTasks" class="empty-state">
        <svg>...</svg>
        <p>暂无后台任务</p>
      </div>
    </div>

    <!-- 滚动区：任务分组 -->
    <div class="monitor-body" ref="scrollContainer">
      <!-- SSE 断开警告 -->
      <div v-if="!sseConnected" class="sse-alert">
        ⚠️ 实时连接已断开，正在尝试重连...
      </div>

      <!-- 失败任务 -->
      <TaskGroup
        title="失败"
        :count="failedTasks.length"
        variant="danger"
        :defaultCollapsed="true"
      >
        <TaskCard
          v-for="task in failedTasks"
          :key="task.job_id"
          :task="task"
        />
      </TaskGroup>

      <!-- 排队中（可拖动） -->
      <TaskGroup
        title="排队中"
        :count="queuedTasks.length"
        variant="primary"
        :defaultCollapsed="false"
      >
        <Draggable
          v-model="queuedTasks"
          item-key="job_id"
          :animation="200"
          ghost-class="task-ghost"
          handle=".drag-handle"
          :scroll-sensitivity="80"
          :scroll-speed="20"
          @end="handleDragEnd"
        >
          <template #item="{ element }">
            <TaskCard :task="element" :draggable="true" />
          </template>
        </Draggable>
      </TaskGroup>

      <!-- 已暂停 -->
      <TaskGroup
        title="已暂停"
        :count="pausedTasks.length"
        variant="warning"
        :defaultCollapsed="true"
      >
        <TaskCard
          v-for="task in pausedTasks"
          :key="task.job_id"
          :task="task"
        />
      </TaskGroup>

      <!-- 最近完成 -->
      <TaskGroup
        title="最近完成"
        :count="finishedTasks.length"
        variant="success"
        :defaultCollapsed="true"
      >
        <TransitionGroup name="list">
          <TaskCard
            v-for="task in recentFinishedTasks"
            :key="task.job_id"
            :task="task"
            :class="{ 'newly-finished': task.isNewlyFinished }"
          />
        </TransitionGroup>

        <div class="view-more" @click="openHistoryPage">
          查看全部历史记录 >
        </div>
      </TaskGroup>
    </div>

    <!-- 底部渐变遮罩 -->
    <div class="gradient-mask"></div>
  </div>
</template>

<script setup>
import { computed, ref, onMounted, onUnmounted } from 'vue'
import { useUnifiedTaskStore } from '@/stores/unifiedTaskStore'
import Draggable from 'vuedraggable'
import TaskGroup from './TaskGroup.vue'
import TaskCard from './TaskCard.vue'

const taskStore = useUnifiedTaskStore()

// 从 store 获取数据
const processingTask = computed(() => taskStore.processingTask)
const queuedTasks = computed(() => taskStore.queuedTasks)
const failedTasks = computed(() => taskStore.failedTasks)
const pausedTasks = computed(() => taskStore.pausedTasks)
const recentFinishedTasks = computed(() => taskStore.recentFinishedTasks)
const sseConnected = computed(() => taskStore.sseConnected)

const hasAnyTasks = computed(() =>
  taskStore.tasks.length > 0
)

// 拖动结束处理
async function handleDragEnd(event) {
  if (event.oldIndex === event.newIndex) return

  const newOrder = queuedTasks.value.map(t => t.job_id)
  await taskStore.reorderQueue(newOrder)
}

// 打开历史记录页面
function openHistoryPage() {
  // 跳转到独立的历史记录页面
  router.push('/history')
}

// SSE 心跳检测
let heartbeatTimer = null
onMounted(() => {
  heartbeatTimer = setInterval(() => {
    taskStore.checkSSEConnection()
  }, 5000)
})

onUnmounted(() => {
  if (heartbeatTimer) {
    clearInterval(heartbeatTimer)
  }
})
</script>

<style lang="scss" scoped>
.task-monitor {
  display: flex;
  flex-direction: column;
  height: 100%;
  background: var(--bg-secondary);
  position: relative;
}

.monitor-header {
  flex-shrink: 0;
  background: var(--bg-primary);
  border-bottom: 1px solid var(--border-default);
  max-height: 300px;
  overflow-y: auto;
  padding: 12px;
  box-shadow: var(--shadow-sm);
  z-index: 10;
}

.monitor-body {
  flex: 1;
  overflow-y: auto;
  padding: 12px;
  padding-bottom: 64px;  // 为渐变遮罩留空间

  &::-webkit-scrollbar {
    width: 8px;
  }
  &::-webkit-scrollbar-thumb {
    background: var(--border-muted);
    border-radius: 4px;
  }
}

.gradient-mask {
  position: absolute;
  bottom: 0;
  left: 0;
  right: 0;
  height: 48px;
  background: linear-gradient(to top, var(--bg-secondary), transparent);
  pointer-events: none;
}

.sse-alert {
  background: rgba(248, 81, 73, 0.1);
  color: var(--danger);
  padding: 12px;
  border-radius: 6px;
  margin-bottom: 12px;
  font-size: 13px;
  display: flex;
  align-items: center;
  gap: 8px;
}

.empty-state {
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 48px 24px;
  color: var(--text-muted);

  svg {
    width: 48px;
    height: 48px;
    opacity: 0.5;
    margin-bottom: 12px;
  }

  p {
    font-size: 13px;
    margin: 0;
  }
}

.view-more {
  text-align: center;
  padding: 8px;
  font-size: 12px;
  color: var(--text-muted);
  cursor: pointer;

  &:hover {
    color: var(--primary);
  }
}

// Vue TransitionGroup 动画
.list-move,
.list-enter-active,
.list-leave-active {
  transition: all 0.3s ease;
}

.list-enter-from,
.list-leave-to {
  opacity: 0;
  transform: translateY(-10px);
}

.list-leave-active {
  position: absolute;
  width: 100%;
}

// 新完成任务高亮
.newly-finished {
  animation: highlight 2s ease-out;
}

@keyframes highlight {
  0%, 100% { background: transparent; }
  50% { background: rgba(63, 185, 80, 0.15); }
}
</style>
```

#### 4.2.2 TaskGroup (折叠组件)

```vue
<template>
  <div class="task-group" :class="`variant-${variant}`">
    <!-- 标题栏 -->
    <div class="group-header" @click="toggleCollapse">
      <div class="header-left">
        <span class="status-dot"></span>
        <span class="group-title">{{ title }}</span>
        <span class="group-count">({{ count }})</span>
      </div>
      <div class="header-right">
        <svg
          class="collapse-icon"
          :class="{ collapsed: isCollapsed }"
          viewBox="0 0 24 24"
        >
          <path d="M7 10l5 5 5-5z"/>
        </svg>
      </div>
    </div>

    <!-- 内容区（使用 CSS Grid 折叠） -->
    <div
      class="group-content-wrapper"
      :class="{ collapsed: isCollapsed }"
    >
      <div class="group-content">
        <slot></slot>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'

const props = defineProps({
  title: String,
  count: { type: Number, default: 0 },
  variant: {
    type: String,
    default: 'default',
    validator: (v) => ['default', 'primary', 'success', 'warning', 'danger'].includes(v)
  },
  defaultCollapsed: { type: Boolean, default: false }
})

const isCollapsed = ref(props.defaultCollapsed)

function toggleCollapse() {
  isCollapsed.value = !isCollapsed.value
}
</script>

<style lang="scss" scoped>
.task-group {
  background: var(--bg-tertiary);
  border: 1px solid var(--border-default);
  border-radius: 8px;
  margin-bottom: 12px;
  overflow: hidden;
}

.group-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 12px 16px;
  cursor: pointer;
  user-select: none;
  transition: background 0.2s;

  &:hover {
    background: var(--bg-elevated);
  }

  .header-left {
    display: flex;
    align-items: center;
    gap: 8px;
  }

  .status-dot {
    width: 8px;
    height: 8px;
    border-radius: 50%;
    background: var(--text-muted);
  }

  .group-title {
    font-size: 14px;
    font-weight: 600;
    color: var(--text-primary);
  }

  .group-count {
    font-size: 12px;
    color: var(--text-muted);
  }

  .collapse-icon {
    width: 20px;
    height: 20px;
    color: var(--text-muted);
    transition: transform 0.3s ease;

    &.collapsed {
      transform: rotate(-90deg);
    }
  }
}

// CSS Grid 折叠动画
.group-content-wrapper {
  display: grid;
  grid-template-rows: 1fr;
  transition: grid-template-rows 300ms ease-out;

  &.collapsed {
    grid-template-rows: 0fr;
  }
}

.group-content {
  overflow: hidden;
  padding: 0 12px 12px;
}

// 变体样式
.variant-primary .status-dot { background: var(--primary); }
.variant-success .status-dot { background: var(--success); }
.variant-warning .status-dot { background: var(--warning); }
.variant-danger .status-dot { background: var(--danger); }
</style>
```

#### 4.2.3 TaskCard (任务卡片)

```vue
<template>
  <div
    class="task-card"
    :class="[
      `variant-${variant}`,
      { 'is-draggable': draggable }
    ]"
  >
    <!-- 拖动手柄 -->
    <div v-if="draggable" class="drag-handle" title="拖动排序">
      <svg viewBox="0 0 24 24">
        <path d="M11 18c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zm-2-8c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0-6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm6 4c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/>
      </svg>
    </div>

    <!-- 任务信息 -->
    <div class="task-info">
      <div class="task-header">
        <span class="task-name">{{ task.title || task.filename }}</span>
        <span
          class="task-phase"
          :style="{
            background: getPhaseStyle(task).bgColor,
            color: getPhaseStyle(task).color
          }"
        >
          {{ getPhaseLabel(task) }}
        </span>
      </div>

      <!-- 进度条 -->
      <div v-if="showProgress" class="task-progress">
        <div class="progress-bar">
          <div
            class="progress-fill"
            :style="{ width: task.progress + '%' }"
          ></div>
        </div>
        <span class="progress-text">{{ task.progress.toFixed(1) }}%</span>
      </div>

      <!-- 错误信息 -->
      <div v-if="task.status === 'failed'" class="task-error">
        <svg viewBox="0 0 24 24">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
        </svg>
        <span>{{ task.message || '转录失败' }}</span>
      </div>

      <!-- 完成时间 -->
      <div v-if="task.status === 'finished'" class="task-meta">
        <svg viewBox="0 0 24 24">
          <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/>
        </svg>
        <span>{{ formatTime(task.completed_at) }}</span>
      </div>
    </div>

    <!-- 操作按钮 -->
    <div class="task-actions">
      <button
        v-if="task.status === 'processing'"
        class="action-btn"
        @click="pauseTask"
        title="暂停"
      >
        <svg viewBox="0 0 24 24">
          <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>
        </svg>
      </button>

      <button
        v-if="task.status === 'paused'"
        class="action-btn action-btn--success"
        @click="resumeTask"
        title="恢复"
      >
        <svg viewBox="0 0 24 24">
          <path d="M8 5v14l11-7z"/>
        </svg>
      </button>

      <button
        v-if="['processing', 'queued', 'paused'].includes(task.status)"
        class="action-btn action-btn--danger"
        @click="cancelTask"
        title="取消"
      >
        <svg viewBox="0 0 24 24">
          <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
        </svg>
      </button>

      <button
        v-if="task.status === 'finished'"
        class="action-btn action-btn--primary"
        @click="openEditor"
        title="打开编辑器"
      >
        <svg viewBox="0 0 24 24">
          <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>
        </svg>
      </button>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue'
import { useRouter } from 'vue-router'
import { transcriptionApi } from '@/services/api'
import { PHASE_CONFIG, STATUS_CONFIG } from '@/constants/taskPhases'

const props = defineProps({
  task: { type: Object, required: true },
  variant: { type: String, default: 'default' },
  draggable: { type: Boolean, default: false }
})

const router = useRouter()

const showProgress = computed(() =>
  ['processing', 'queued', 'paused'].includes(props.task.status)
)

function getPhaseStyle(task) {
  if (task.status === 'failed') return STATUS_CONFIG.failed
  if (task.status === 'processing' && task.phase) {
    return PHASE_CONFIG[task.phase] || PHASE_CONFIG.pending
  }
  return STATUS_CONFIG[task.status] || STATUS_CONFIG.created
}

function getPhaseLabel(task) {
  if (task.status === 'failed') return '失败'
  if (task.status === 'processing' && task.phase) {
    return PHASE_CONFIG[task.phase]?.label || '处理中'
  }
  return STATUS_CONFIG[task.status]?.label || task.status
}

async function pauseTask() {
  await transcriptionApi.pauseJob(props.task.job_id)
}

async function resumeTask() {
  await transcriptionApi.resumeJob(props.task.job_id)
}

async function cancelTask() {
  if (!confirm('确定要取消这个任务吗?')) return
  await transcriptionApi.cancelJob(props.task.job_id, false)
}

function openEditor() {
  router.push(`/editor/${props.task.job_id}`)
}

function formatTime(timestamp) {
  if (!timestamp) return ''
  const date = new Date(timestamp)
  const now = new Date()
  const diff = now - date

  if (diff < 60000) return '刚刚'
  if (diff < 3600000) return `${Math.floor(diff / 60000)} 分钟前`
  if (diff < 86400000) return `${Math.floor(diff / 3600000)} 小时前`
  return date.toLocaleDateString('zh-CN')
}
</script>

<style lang="scss" scoped>
.task-card {
  display: flex;
  gap: 12px;
  padding: 12px;
  background: var(--bg-primary);
  border: 1px solid var(--border-default);
  border-radius: 6px;
  margin-bottom: 8px;
  transition: all 0.2s;

  &:hover {
    background: var(--bg-elevated);
  }

  &.is-draggable {
    cursor: move;
  }
}

.drag-handle {
  width: 20px;
  display: flex;
  align-items: center;
  justify-content: center;
  color: var(--text-muted);
  cursor: grab;
  opacity: 0.5;

  &:hover {
    opacity: 1;
  }

  &:active {
    cursor: grabbing;
  }

  svg {
    width: 16px;
    height: 16px;
  }
}

.task-info {
  flex: 1;
  min-width: 0;
}

.task-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  margin-bottom: 8px;
  gap: 8px;
}

.task-name {
  flex: 1;
  font-size: 13px;
  font-weight: 500;
  color: var(--text-primary);
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.task-phase {
  padding: 2px 6px;
  border-radius: 4px;
  font-size: 10px;
  font-weight: 600;
  white-space: nowrap;
}

.task-progress {
  display: flex;
  align-items: center;
  gap: 8px;
}

.progress-bar {
  flex: 1;
  height: 4px;
  background: var(--border-muted);
  border-radius: 2px;
  overflow: hidden;
}

.progress-fill {
  height: 100%;
  background: var(--primary);
  transition: width 0.3s ease;
}

.progress-text {
  font-size: 11px;
  font-family: var(--font-mono);
  color: var(--text-muted);
  min-width: 35px;
  text-align: right;
}

.task-error {
  display: flex;
  align-items: center;
  gap: 6px;
  font-size: 11px;
  color: var(--danger);

  svg {
    width: 14px;
    height: 14px;
  }
}

.task-meta {
  display: flex;
  align-items: center;
  gap: 6px;
  font-size: 11px;
  color: var(--text-muted);

  svg {
    width: 14px;
    height: 14px;
    color: var(--success);
  }
}

.task-actions {
  display: flex;
  flex-direction: column;
  gap: 4px;
}

.action-btn {
  width: 28px;
  height: 28px;
  display: flex;
  align-items: center;
  justify-content: center;
  background: transparent;
  border: none;
  border-radius: 4px;
  color: var(--text-muted);
  cursor: pointer;
  transition: all 0.2s;

  svg {
    width: 16px;
    height: 16px;
  }

  &:hover {
    background: var(--bg-tertiary);
    color: var(--text-secondary);
  }

  &--primary {
    color: var(--primary);
    &:hover {
      background: rgba(88, 166, 255, 0.15);
    }
  }

  &--success {
    color: var(--success);
    &:hover {
      background: rgba(63, 185, 80, 0.15);
    }
  }

  &--danger:hover {
    background: rgba(248, 81, 73, 0.15);
    color: var(--danger);
  }
}

// 拖动占位符样式
.task-ghost {
  opacity: 0.5;
  background: var(--bg-tertiary);
  border: 2px dashed var(--border-default);
}
</style>
```

---

## 5. 技术实现方案

### 5.1 Store 实现 (unifiedTaskStore.js)

```javascript
import { defineStore } from 'pinia'
import { ref, computed, reactive } from 'vue'
import { transcriptionApi } from '@/services/api'

export const useUnifiedTaskStore = defineStore('unifiedTask', () => {
  // ========== 核心数据 ==========
  const tasksMap = reactive(new Map())  // ID -> Task
  const queueOrder = ref([])            // 队列顺序 [jobId1, jobId2, ...]

  // ========== SSE 状态 ==========
  const sseConnected = ref(false)
  const lastHeartbeat = ref(Date.now())

  // ========== 计算属性 ==========

  // 所有任务（数组形式，用于遍历）
  const tasks = computed(() => Array.from(tasksMap.values()))

  // Processing 任务（最多 1 个）
  const processingTask = computed(() => {
    const processing = tasks.value.filter(t => t.status === 'processing')
    return processing.length > 0 ? processing[0] : null
  })

  // 排队任务（严格按 queueOrder 排序）
  const queuedTasks = computed(() => {
    return queueOrder.value
      .map(id => tasksMap.get(id))
      .filter(t => t && t.status === 'queued')
  })

  // 失败任务（按失败时间倒序）
  const failedTasks = computed(() =>
    tasks.value
      .filter(t => t.status === 'failed')
      .sort((a, b) => (b.failed_at || b.updated_at) - (a.failed_at || a.updated_at))
  )

  // 暂停任务（按暂停时间倒序）
  const pausedTasks = computed(() =>
    tasks.value
      .filter(t => t.status === 'paused')
      .sort((a, b) => (b.paused_at || b.updated_at) - (a.paused_at || a.updated_at))
  )

  // 最近完成任务（最多 20 条）
  const recentFinishedTasks = computed(() =>
    tasks.value
      .filter(t => t.status === 'finished')
      .sort((a, b) => (b.completed_at || b.updated_at) - (a.completed_at || a.updated_at))
      .slice(0, 20)
  )

  // ========== Actions ==========

  /**
   * 添加任务
   */
  function addTask(taskData) {
    const task = {
      job_id: taskData.job_id,
      filename: taskData.filename,
      title: taskData.title || null,
      status: taskData.status || 'created',
      progress: taskData.progress || 0,
      phase: taskData.phase || null,
      phase_percent: taskData.phase_percent || 0,
      message: taskData.message || '',
      created_at: taskData.created_at || Date.now(),
      updated_at: Date.now(),
      completed_at: taskData.completed_at || null,
      paused_at: taskData.paused_at || null,
      failed_at: taskData.failed_at || null,
      sseConnected: false,
      lastError: null,
      isNewlyFinished: false
    }

    tasksMap.set(task.job_id, task)
    saveTasks()
  }

  /**
   * 更新任务
   */
  function updateTask(jobId, updates) {
    const task = tasksMap.get(jobId)
    if (!task) return

    // 检测任务是否刚完成
    if (updates.status === 'finished' && task.status !== 'finished') {
      updates.isNewlyFinished = true
      updates.completed_at = Date.now()

      // 2 秒后移除高亮
      setTimeout(() => {
        const t = tasksMap.get(jobId)
        if (t) {
          t.isNewlyFinished = false
        }
      }, 2000)
    }

    // 更新时间戳
    if (updates.status === 'paused' && task.status !== 'paused') {
      updates.paused_at = Date.now()
    }
    if (updates.status === 'failed' && task.status !== 'failed') {
      updates.failed_at = Date.now()
    }

    Object.assign(task, updates, { updated_at: Date.now() })
    saveTasks()
  }

  /**
   * 更新任务进度
   */
  function updateTaskProgress(jobId, progress, extraData = {}) {
    const task = tasksMap.get(jobId)
    if (!task) return

    task.progress = Math.round(progress * 10) / 10

    if (extraData.phase) task.phase = extraData.phase
    if (extraData.phase_percent !== undefined) {
      task.phase_percent = Math.round(extraData.phase_percent * 10) / 10
    }
    if (extraData.message) task.message = extraData.message

    task.sseConnected = true
    task.lastError = null
    task.updated_at = Date.now()
  }

  /**
   * 删除任务
   */
  function deleteTask(jobId) {
    tasksMap.delete(jobId)
    queueOrder.value = queueOrder.value.filter(id => id !== jobId)
    saveTasks()
  }

  /**
   * 重新排序队列
   */
  async function reorderQueue(newOrder) {
    const oldOrder = [...queueOrder.value]

    // 乐观更新
    queueOrder.value = newOrder

    try {
      const result = await transcriptionApi.reorderQueue(newOrder)
      if (!result.reordered) {
        // 恢复原顺序
        queueOrder.value = oldOrder
        console.error('[TaskStore] 队列重排失败')
      }
    } catch (error) {
      // 恢复原顺序
      queueOrder.value = oldOrder
      console.error('[TaskStore] 队列重排请求失败:', error)
      throw error
    }
  }

  /**
   * 检查 SSE 连接状态
   */
  function checkSSEConnection() {
    if (Date.now() - lastHeartbeat.value > 30000) {  // 30 秒无心跳
      sseConnected.value = false
    }
  }

  /**
   * 更新 SSE 心跳
   */
  function updateSSEHeartbeat() {
    lastHeartbeat.value = Date.now()
    sseConnected.value = true
  }

  /**
   * 同步后端任务列表
   */
  async function syncTasksFromBackend() {
    try {
      const response = await transcriptionApi.syncTasks()
      if (!response.success) return false

      const backendTasks = response.tasks || []
      const backendTaskIds = new Set(backendTasks.map(t => t.id))

      // 删除前端有但后端没有的任务（幽灵任务）
      for (const [localId, task] of tasksMap) {
        if (!backendTaskIds.has(localId)) {
          tasksMap.delete(localId)
        }
      }

      // 更新或添加后端任务
      for (const backendTask of backendTasks) {
        const existingTask = tasksMap.get(backendTask.id)
        if (existingTask) {
          // 更新现有任务
          Object.assign(existingTask, {
            status: backendTask.status,
            progress: backendTask.progress,
            phase: backendTask.phase,
            phase_percent: backendTask.phase_percent || 0,
            message: backendTask.message,
            updated_at: Date.now()
          })
        } else {
          // 添加新任务
          addTask({
            job_id: backendTask.id,
            filename: backendTask.filename,
            title: backendTask.title,
            status: backendTask.status,
            progress: backendTask.progress,
            phase: backendTask.phase,
            phase_percent: backendTask.phase_percent || 0,
            message: backendTask.message,
            created_at: backendTask.created_time || Date.now()
          })
        }
      }

      // 更新队列顺序
      if (response.queue) {
        queueOrder.value = response.queue
      }

      saveTasks()
      return true
    } catch (error) {
      console.error('[TaskStore] 同步任务失败:', error)
      return false
    }
  }

  /**
   * 持久化到 localStorage
   */
  function saveTasks() {
    try {
      const tasksArray = Array.from(tasksMap.values())
      localStorage.setItem('task-list', JSON.stringify(tasksArray))
      localStorage.setItem('queue-order', JSON.stringify(queueOrder.value))
    } catch (error) {
      console.error('[TaskStore] 保存任务失败:', error)
    }
  }

  /**
   * 从 localStorage 恢复
   */
  function restoreTasks() {
    try {
      const saved = localStorage.getItem('task-list')
      if (saved) {
        const tasksArray = JSON.parse(saved)
        tasksMap.clear()
        tasksArray.forEach(t => tasksMap.set(t.job_id, t))
      }

      const savedOrder = localStorage.getItem('queue-order')
      if (savedOrder) {
        queueOrder.value = JSON.parse(savedOrder)
      }
    } catch (error) {
      console.error('[TaskStore] 恢复任务失败:', error)
    }
  }

  // 初始化
  restoreTasks()

  return {
    // 状态
    tasksMap,
    queueOrder,
    sseConnected,
    lastHeartbeat,

    // 计算属性
    tasks,
    processingTask,
    queuedTasks,
    failedTasks,
    pausedTasks,
    recentFinishedTasks,

    // 方法
    addTask,
    updateTask,
    updateTaskProgress,
    deleteTask,
    reorderQueue,
    checkSSEConnection,
    updateSSEHeartbeat,
    syncTasksFromBackend,
    saveTasks,
    restoreTasks
  }
})
```

### 5.2 SSE 集成 (sseChannelManager.js 修改)

```javascript
// 在 subscribeGlobal 中添加心跳处理
subscribeGlobal(handlers = {}) {
  const channelId = 'global'
  const url = `${this.baseURL}/api/events/global`

  return this._subscribe(channelId, url, {
    // ... 其他事件处理器

    ping: () => {
      // 更新心跳时间
      const taskStore = useUnifiedTaskStore()
      taskStore.updateSSEHeartbeat()
    },

    connected: (data) => {
      const taskStore = useUnifiedTaskStore()
      taskStore.updateSSEHeartbeat()
      handlers.onConnected?.(data)
    }
  })
}
```

### 5.3 拖动排序配置

```javascript
// 安装依赖
npm install vuedraggable@next

// 在组件中配置
<Draggable
  v-model="queuedTasks"
  item-key="job_id"
  :animation="200"              // 动画时长
  ghost-class="task-ghost"      // 占位符样式
  handle=".drag-handle"         // 拖动手柄
  :scroll-sensitivity="80"      // 滚动灵敏度
  :scroll-speed="20"            // 滚动速度
  :force-fallback="true"        // 强制使用 fallback（兼容性）
  @end="handleDragEnd"
>
```

---

## 6. 代码修改清单

### 6.1 需要修改的文件

| 文件路径 | 修改类型 | 优先级 |
|---------|---------|--------|
| `frontend/src/stores/unifiedTaskStore.js` | 重构 | P0 |
| `frontend/src/components/editor/TaskMonitor/index.vue` | 重构 | P0 |
| `frontend/src/components/editor/TaskMonitor/TaskGroup.vue` | 新建 | P0 |
| `frontend/src/components/editor/TaskMonitor/TaskCard.vue` | 新建 | P0 |
| `frontend/src/services/sseChannelManager.js` | 修改 | P1 |
| `frontend/src/views/TaskListView.vue` | 修改 | P1 |
| `frontend/package.json` | 添加依赖 | P0 |

### 6.2 详细修改内容

#### 6.2.1 unifiedTaskStore.js

**需要移除的代码：**
```javascript
// ❌ 移除：多任务处理相关
const processingTasks = computed(() =>
  tasks.value.filter(t => t.status === 'processing')  // 返回数组
)

// ❌ 移除：activeCount（系统只允许 1 个任务运行）
const activeCount = computed(() =>
  tasks.value.filter(t => ['queued', 'processing'].includes(t.status)).length
)
```

**需要添加的代码：**
```javascript
// ✅ 添加：单任务处理
const processingTask = computed(() => {
  const processing = tasks.value.filter(t => t.status === 'processing')
  return processing.length > 0 ? processing[0] : null  // 返回单个对象或 null
})

// ✅ 添加：队列顺序索引
const queueOrder = ref([])

// ✅ 添加：严格按队列顺序排序
const queuedTasks = computed(() => {
  return queueOrder.value
    .map(id => tasksMap.get(id))
    .filter(t => t && t.status === 'queued')
})

// ✅ 添加：SSE 心跳检测
const sseConnected = ref(false)
const lastHeartbeat = ref(Date.now())

function checkSSEConnection() {
  if (Date.now() - lastHeartbeat.value > 30000) {
    sseConnected.value = false
  }
}

function updateSSEHeartbeat() {
  lastHeartbeat.value = Date.now()
  sseConnected.value = true
}

// ✅ 添加：任务完成高亮
function updateTask(jobId, updates) {
  const task = tasksMap.get(jobId)
  if (!task) return

  if (updates.status === 'finished' && task.status !== 'finished') {
    updates.isNewlyFinished = true
    setTimeout(() => {
      const t = tasksMap.get(jobId)
      if (t) t.isNewlyFinished = false
    }, 2000)
  }

  Object.assign(task, updates, { updated_at: Date.now() })
}

// ✅ 添加：限制历史记录数量
const recentFinishedTasks = computed(() =>
  tasks.value
    .filter(t => t.status === 'finished')
    .sort((a, b) => b.completed_at - a.completed_at)
    .slice(0, 20)  // 只取前 20 条
)
```

#### 6.2.2 TaskMonitor/index.vue

**完全重构，主要变更：**

1. **布局结构**
   ```vue
   <!-- 旧版：所有任务混在一起 -->
   <div class="task-list">
     <div v-for="task in filteredTasks" ...>
   </div>
   
   <!-- 新版：分区域布局 -->
   <div class="monitor-header">
     <!-- Processing 任务区域（固定） -->
   </div>
   <div class="monitor-body">
     <!-- 任务分组区域（可滚动） -->
   </div>
   <div class="gradient-mask"></div>
   ```

2. **数据绑定**
   ```javascript
   // 旧版
   const filteredTasks = computed(() =>
     taskStore.tasks.filter(t => t.status !== 'created')
   )
   
   // 新版
   const processingTask = computed(() => taskStore.processingTask)
   const queuedTasks = computed(() => taskStore.queuedTasks)
   const failedTasks = computed(() => taskStore.failedTasks)
   const pausedTasks = computed(() => taskStore.pausedTasks)
   const recentFinishedTasks = computed(() => taskStore.recentFinishedTasks)
   ```

3. **拖动排序**
   ```vue
   <!-- 旧版：直接在任务列表上拖动 -->
   <draggable v-model="queuedTasks" ...>
   
   <!-- 新版：只在 Queued 分组内拖动 -->
   <TaskGroup title="排队中">
     <Draggable v-model="queuedTasks" ...>
   </TaskGroup>
   ```

#### 6.2.3 新建组件

**TaskGroup.vue**
- 实现折叠展开功能
- 使用 CSS Grid 动画
- 支持不同的视觉变体（primary/success/warning/danger）

**TaskCard.vue**
- 从 TaskMonitor/index.vue 中提取任务卡片逻辑
- 支持拖动手柄
- 支持不同的显示模式（processing/queued/finished）

#### 6.2.4 sseChannelManager.js

**添加心跳处理：**
```javascript
subscribeGlobal(handlers = {}) {
  return this._subscribe(channelId, url, {
    // ... 现有事件处理器

    ping: () => {
      // 新增：更新心跳时间
      const taskStore = useUnifiedTaskStore()
      taskStore.updateSSEHeartbeat()
    },

    connected: (data) => {
      // 新增：连接成功时更新心跳
      const taskStore = useUnifiedTaskStore()
      taskStore.updateSSEHeartbeat()
      handlers.onConnected?.(data)
    }
  })
}
```

---

## 7. 实施步骤

### 阶段 1：数据层重构（1 天）

**目标：** 建立稳定的数据基础，确保单任务运行约束

**任务清单：**
- [x] 修改 `unifiedTaskStore.js`
  - [x] 将 `processingTasks` 改为 `processingTask`（单例）
  - [x] 添加 `queueOrder` 索引
  - [x] 重构 `queuedTasks` 计算属性（按顺序排序）
  - [x] 添加 SSE 心跳检测机制
  - [x] 添加 `recentFinishedTasks`（限制 20 条）
  - [x] 实现 `updateSSEHeartbeat()` 方法
  - [x] 持久化 `queueOrder`

**验收标准：**

```javascript
// 测试代码
const store = useUnifiedTaskStore()

// 1. Processing 任务应该是单个对象或 null
console.assert(
  store.processingTask === null || typeof store.processingTask === 'object',
  'processingTask 应该是单个对象或 null'
)

// 2. queuedTasks 应该按 queueOrder 排序
const queuedIds = store.queuedTasks.map(t => t.job_id)
console.assert(
  JSON.stringify(queuedIds) === JSON.stringify(store.queueOrder),
  'queuedTasks 顺序应该与 queueOrder 一致'
)

// 3. 历史记录应该限制数量
console.assert(
  store.recentFinishedTasks.length <= 20,
  '历史记录应该最多 20 条'
)
```

---

### 阶段 2：组件化与布局（1 天）

**目标：** 实现固定头部 + 可滚动主体的布局，引入折叠功能

**任务清单：**
- [x] 创建 `TaskGroup.vue` 组件
  - [x] 实现 CSS Grid 折叠动画
  - [x] 支持默认折叠/展开
  - [x] 支持视觉变体（primary/success/warning/danger）
  - [x] 点击标题栏切换折叠状态

- [x] 创建 `TaskCard.vue` 组件
  - [x] 从 `TaskMonitor/index.vue` 提取任务卡片 UI
  - [x] 支持拖动手柄（通过 `draggable` prop 控制）
  - [x] 实现任务操作按钮（暂停/恢复/取消/打开）

- [x] 重构 `TaskMonitor/index.vue`
  - [x] 实现固定 Header + 滚动 Body 布局
  - [x] 渲染 Processing 区域（单个任务或空状态）
  - [x] 使用 `TaskGroup` 组件渲染任务分组
  - [x] 添加底部渐变遮罩
  - [x] 添加 SSE 断开警告

**验收标准：**

- [ ] Processing 区域固定在顶部，不随主体滚动
- [ ] 点击分组标题可以折叠/展开
- [ ] 折叠动画流畅（300ms）
- [ ] 底部渐变遮罩始终可见
- [ ] SSE 断开时显示警告横幅

---

### 阶段 3：拖动排序集成（0.5 天）

**目标：** 恢复队列拖动排序功能，优化手感

**任务清单：**

- [x] 安装 `vuedraggable@next`
  ```bash
  npm install vuedraggable@next
  ```

- [x] 在 `TaskMonitor/index.vue` 中集成 `Draggable`
  - [x] 仅在 Queued 分组内启用拖动
  - [x] 配置滚动热区（`scrollSensitivity: 80`, `scrollSpeed: 20`）
  - [x] 绑定 `@end` 事件到 `store.reorderQueue()`

- [x] 优化拖动样式
  - [x] 添加 `.task-ghost` 占位符样式
  - [x] 拖动手柄悬停效果
  - [x] 拖动时改变光标（`cursor: grabbing`）

**验收标准：**
- [ ] 拖动任务卡片可以改变队列顺序
- [ ] 拖动到视口边缘时自动滚动
- [ ] 占位符清晰可见（虚线边框）
- [ ] 拖动结束后调用后端 API 同步顺序
- [ ] API 失败时恢复原顺序

---

### 阶段 4：SSE 心跳与细节优化（0.5 天）

**目标：** 增强 SSE 稳定性，优化用户体验

**任务清单：**
- [x] 在 `sseChannelManager.js` 中集成心跳处理
  - [x] `ping` 事件调用 `taskStore.updateSSEHeartbeat()`
  - [x] `connected` 事件调用 `taskStore.updateSSEHeartbeat()`

- [x] 在 `TaskMonitor/index.vue` 中启动心跳检测定时器
  ```javascript
  onMounted(() => {
    heartbeatTimer = setInterval(() => {
      taskStore.checkSSEConnection()
    }, 5000)  // 每 5 秒检测一次
  })
  ```

- [x] 添加任务完成高亮动画
  - [x] 在 `updateTask` 中设置 `isNewlyFinished = true`
  - [x] 2 秒后自动移除标记
  - [x] 使用 CSS `@keyframes highlight` 实现闪烁效果

- [x] 优化空状态显示
  - [x] Processing 区域无任务时显示提示
  - [x] 分组内无任务时显示"暂无任务"

**验收标准：**
- [ ] SSE 断开 30 秒后显示警告横幅
- [ ] SSE 重连后警告自动消失
- [ ] 任务完成时卡片短暂高亮（绿色背景闪烁）
- [ ] 空状态显示友好提示信息

---

### 阶段 5：集成测试与修复（0.5 天）

**目标：** 端到端测试，修复边界情况

**测试用例：**
1. **单任务运行验证**
   - [ ] Processing 区域最多显示 1 个任务
   - [ ] 任务完成后，下一个排队任务自动开始

2. **拖动排序**
   - [ ] 拖动任务改变队列顺序
   - [ ] 后端 API 失败时恢复原顺序
   - [ ] 拖动期间 SSE 更新不干扰顺序

3. **折叠功能**
   - [ ] 点击标题栏切换折叠
   - [ ] 刷新页面后保持折叠状态（可选）
   - [ ] 折叠动画流畅，无卡顿

4. **SSE 稳定性**
   - [ ] 断开网络 30 秒后显示警告
   - [ ] 恢复网络后自动重连
   - [ ] 心跳正常时 `sseConnected` 为 `true`

5. **历史记录限制**
   - [ ] 完成 30 个任务后，只显示最近 20 个
   - [ ] "查看更多"按钮跳转到历史页面

---

## 8. 验收标准

### 8.1 功能验收

| 功能点 | 验收标准 | 优先级 |
|--------|---------|--------|
| 单任务运行 | Processing 区域最多显示 1 个任务 | P0 |
| 任务分组 | 按状态分为 Failed/Queued/Paused/Finished 4 组 | P0 |
| 折叠功能 | 点击标题栏可切换折叠/展开 | P0 |
| 拖动排序 | Queued 任务可拖动改变顺序 | P0 |
| 历史限制 | Finished 分组最多显示 20 条 | P1 |
| SSE 检测 | 断开 30 秒后显示警告 | P1 |
| 任务高亮 | 完成时短暂高亮（2 秒） | P2 |
| 底部遮罩 | 始终显示渐变遮罩 | P2 |

### 8.2 性能验收

| 指标 | 标准 | 测试方法 |
|------|------|---------|
| 折叠动画 | < 300ms | Chrome DevTools Performance |
| 拖动响应 | < 100ms | 手动测试，无明显延迟 |
| 列表渲染 | 50 个任务 < 500ms | 模拟 50 个任务，测试首屏渲染 |
| SSE 心跳 | 每 30s 检测一次 | 断开网络，观察警告出现时间 |

### 8.3 兼容性验收

| 浏览器 | 版本 | 验收结果 |
|--------|------|---------|
| Chrome | >= 90 | ✅ |
| Edge | >= 90 | ✅ |
| Firefox | >= 88 | ✅ |
| Safari | >= 14 | ⚠️ CSS Grid 动画可能有差异 |

---

## 9. 风险与应对

### 9.1 技术风险

| 风险 | 影响 | 概率 | 应对措施 |
|------|------|------|---------|
| CSS Grid 折叠在旧浏览器不兼容 | 中 | 低 | 使用 `max-height` Fallback |
| 拖动与 SSE 更新冲突 | 高 | 中 | 拖动期间暂停 SSE 更新 |
| 历史记录过多导致卡顿 | 中 | 低 | 限制显示数量（已实现） |
| SSE 心跳检测误报 | 低 | 中 | 延长超时时间至 60 秒 |

### 9.2 迁移风险

| 风险 | 影响 | 应对措施 |
|------|------|---------|
| 现有用户习惯改变 | 中 | 保持核心操作一致，添加引导提示 |
| localStorage 数据格式变更 | 高 | 实现数据迁移脚本 |
| 旧版组件依赖 | 低 | 保留旧版组件，逐步废弃 |

---

## 10. 后续优化方向

1. **虚拟滚动**（如果历史记录需要显示 100+ 条）
   - 引入 `vue-virtual-scroller`
   - 仅在独立的历史记录页面使用

2. **任务分组折叠状态持久化**
   - 将折叠状态保存到 localStorage
   - 刷新页面后保持状态

3. **高级过滤与搜索**
   - 按文件名搜索任务
   - 按时间范围筛选
   - 按状态快速过滤

4. **任务统计面板**
   - 显示成功率、平均处理时间
   - 显示队列等待时间预估

5. **拖动优化**
   - 支持多选任务批量排序
   - 支持键盘快捷键（Ctrl+↑/↓）

---

## 11. 附录

### 11.1 相关文档

- [任务监控器 UI 系统设计文档.md](./任务监控器%20UI%20系统设计文档.md)
- [任务监视2.md](./任务监视2.md)
- [UnifiedTaskStore.md](./组件/UnifiedTaskStore.md)

### 11.2 依赖库

```json
{
  "dependencies": {
    "vue": "^3.3.0",
    "pinia": "^2.1.0",
    "vue-router": "^4.2.0",
    "vuedraggable": "^4.1.0"
  }
}
```

### 11.3 CSS 变量

```css
:root {
  /* 颜色 */
  --primary: #58a6ff;
  --success: #3fb950;
  --warning: #d29922;
  --danger: #f85149;

  /* 背景 */
  --bg-primary: #ffffff;
  --bg-secondary: #f6f8fa;
  --bg-tertiary: #f0f0f0;
  --bg-elevated: #ffffff;

  /* 边框 */
  --border-default: #d0d7de;
  --border-muted: #e5e5e5;

  /* 文字 */
  --text-primary: #24292f;
  --text-secondary: #57606a;
  --text-muted: #8b949e;

  /* 阴影 */
  --shadow-sm: 0 1px 2px rgba(0, 0, 0, 0.05);
}
```

---

**文档版本：** 1.0
**最后更新：** 2025-01-XX
**维护者：** 开发团队
