# Video-to-SRT GPU 项目重构计划 (实战版)

> **核心原则**: 先稳后快，黑盒测试护航，聚焦断点续传
> **预计耗时**: 3-5天
> **当前项目版本**: v2.0
> **当前分支**: feature/breakpoint-resume
> **重构目标**: 统一逻辑、实现断点续传、支持独立打包

---

## 📋 目录

- [一、为什么重构？](#一为什么重构)
- [二、重构前的"救命索"](#二重构前的救命索)
- [三、分步执行计划](#三分步执行计划)
- [四、风险控制](#四风险控制)
- [五、验收标准](#五验收标准)

---

## 一、为什么重构？

### 1.1 当前痛点

#### 问题1: 入口文件混乱 ⚠️⚠️⚠️
**现状**:
```
backend/app/
├── main.py                # 当前主入口？
├── main_refactored.py     # 重构版本？
├── main_simple.py         # 简化版本？
└── debug_main.py          # 调试版本？
```

**问题**: 4个入口文件，不清楚该运行哪个，过段时间自己都忘了。

---

#### 问题2: 转录逻辑重复 ⚠️⚠️⚠️
**现状**:
- `processor.py` (534行) - 包含完整转录流程
- `services/transcription_service.py` (405行) - 几乎相同的实现

**代码重复率**: >90%

**问题**: 改一处还得改另一处，容易漏改导致Bug，且难以实现断点续传。

---

#### 问题3: 路径硬编码 ⚠️⚠️
**现状**: 代码中存在硬编码路径，不利于独立打包

**问题**:
- 无法一键切换到嵌入式环境
- 依赖系统环境变量
- 模型下载路径不可控

---

### 1.2 重构目标

1. **统一逻辑** - 只保留一套转录代码
2. **目录整洁** - 清晰的分层结构
3. **功能落地** - 实现断点续传
4. **独立打包** - 支持便携式部署

---

## 二、重构前的"救命索"

### 2.1 黄金标准测试 (Golden Master Test)

**目的**: 确保重构不改坏功能

**准备工作** (30分钟):

1. **准备测试文件**
```bash
# 找一个30秒-1分钟的测试视频
# 放到: tests/fixtures/test_video.mp4
```

2. **生成黄金标准**
```bash
# 用当前未修改的版本运行
python backend/app/main.py

# 上传 test_video.mp4，生成字幕
# 保存结果为: tests/fixtures/golden_standard.srt
```

3. **创建重构分支**
```bash
# 基于当前分支创建重构分支
git checkout -b refactor/clean-architecture

# 创建备份标签
git tag backup-before-refactor-$(date +%Y%m%d)
git push origin backup-before-refactor-$(date +%Y%m%d)
```

**验证脚本**:
```python
# tests/test_golden_master.py
"""
黄金标准测试
确保重构后生成的字幕与重构前一致
"""
import os
import difflib
from pathlib import Path

def load_srt(path):
    """加载SRT文件"""
    with open(path, 'r', encoding='utf-8') as f:
        return f.read()

def compare_srt(golden_path, new_path, tolerance_ms=100):
    """
    比较两个SRT文件

    Args:
        golden_path: 黄金标准文件路径
        new_path: 新生成的文件路径
        tolerance_ms: 时间戳容差(毫秒)

    Returns:
        bool: 是否一致
    """
    golden = load_srt(golden_path)
    new = load_srt(new_path)

    # 简单对比：内容完全一致
    if golden == new:
        print("✅ 完全一致")
        return True

    # 详细对比：显示差异
    diff = difflib.unified_diff(
        golden.splitlines(keepends=True),
        new.splitlines(keepends=True),
        fromfile='golden',
        tofile='new'
    )

    diff_text = ''.join(diff)
    if diff_text:
        print("⚠️ 存在差异:")
        print(diff_text)
        return False

    return True

# 使用示例
if __name__ == "__main__":
    golden = Path("tests/fixtures/golden_standard.srt")
    new = Path("output/test_video.srt")

    if compare_srt(golden, new):
        print("✅ 黄金标准测试通过")
    else:
        print("❌ 黄金标准测试失败")
        exit(1)
```

---

## 三、分步执行计划

### 阶段1: 大扫除与合并 (Day 1)

#### 步骤1.1: 清理入口文件 (1小时)

**目标**: 只保留一个main.py

**操作步骤**:

1. **确定主入口**
```bash
# 检查各个main文件的差异
diff backend/app/main.py backend/app/main_simple.py
diff backend/app/main.py backend/app/main_refactored.py
```

2. **确认main.py是最完整的版本**
```python
# backend/app/main.py 应该包含:
# - FastAPI应用定义
# - 所有API路由
# - 模型预加载管理
# - CORS配置
# - 启动/关闭事件处理
```

3. **归档其他main文件**
```bash
# 创建归档目录
mkdir -p _deprecated

# 移动废弃文件
mv backend/app/main_refactored.py _deprecated/
mv backend/app/main_simple.py _deprecated/
mv backend/app/debug_main.py _deprecated/

# 添加说明文件
cat > _deprecated/README.md << 'EOF'
# 废弃文件归档

这些文件已被废弃，仅保留作为历史参考。

- main_refactored.py - 早期重构尝试
- main_simple.py - 简化版本
- debug_main.py - 调试版本

**当前唯一入口**: backend/app/main.py
EOF
```

4. **更新启动脚本**
```python
# simple_launcher.py
# 确保指向唯一的main.py

# 修改前
backend_cmd = [sys.executable, "-m", "uvicorn", "backend.app.main_simple:app", ...]

# 修改后
backend_cmd = [sys.executable, "-m", "uvicorn", "backend.app.main:app", ...]
```

5. **提交变更**
```bash
git add .
git commit -m "refactor: 统一入口文件，只保留main.py"
```

---

#### 步骤1.2: 合并核心逻辑 (4小时) 🔥

**目标**: 消除processor.py和transcription_service.py的重复

**策略**: 以 `services/transcription_service.py` 为主，整合所有功能

**详细操作**:

**1. 分析差异**
```python
# processor.py 独有功能:
# - CPUAffinityManager (CPU亲和性管理)
# - 模型管理器集成 (get_model_manager)
# - 全局单例模式 (get_processor)

# transcription_service.py 独有功能:
# - 硬件检测集成
# - 更清晰的服务层设计
```

**2. 创建CPU亲和性服务**
```python
# backend/app/services/cpu_affinity_service.py
"""
CPU亲和性管理服务
从processor.py提取
"""

import os
import logging
import platform
from dataclasses import dataclass, field
from typing import List, Optional

try:
    import psutil
except ImportError:
    psutil = None

@dataclass
class CPUAffinityConfig:
    """CPU亲和性配置"""
    enabled: bool = True
    strategy: str = "auto"  # "auto", "half", "custom"
    custom_cores: Optional[List[int]] = None
    exclude_cores: Optional[List[int]] = None

class CPUAffinityManager:
    """
    CPU亲和性管理器
    通过绑定特定CPU核心来优化性能
    """

    def __init__(self):
        self.logger = logging.getLogger(__name__)
        self.original_affinity = None
        self.is_supported = psutil is not None and hasattr(psutil.Process(), 'cpu_affinity')

        if not self.is_supported:
            self.logger.warning("⚠️ CPU亲和性功能不可用：psutil未安装或系统不支持")

    def get_system_info(self) -> dict:
        """
        获取系统CPU信息

        Returns:
            dict: CPU信息
                - supported: bool, 是否支持CPU亲和性
                - logical_cores: int, 逻辑核心数
                - physical_cores: int, 物理核心数
                - current_affinity: List[int], 当前亲和性设置
                - platform: str, 操作系统平台
        """
        if not self.is_supported:
            return {"supported": False, "reason": "psutil not available"}

        try:
            cpu_count = psutil.cpu_count(logical=True)
            physical_count = psutil.cpu_count(logical=False)
            current_affinity = psutil.Process().cpu_affinity()

            return {
                "supported": True,
                "logical_cores": cpu_count,
                "physical_cores": physical_count,
                "current_affinity": current_affinity,
                "platform": platform.system()
            }
        except Exception as e:
            return {"supported": False, "error": str(e)}

    def calculate_optimal_cores(
        self,
        strategy: str = "auto",
        custom_cores: Optional[List[int]] = None,
        exclude_cores: Optional[List[int]] = None
    ) -> List[int]:
        """
        计算最佳CPU核心分配

        Args:
            strategy: 分配策略
                - "auto": 自动根据核心数智能分配
                - "half": 使用前50%核心
                - "custom": 使用自定义核心列表
            custom_cores: 自定义核心列表 (strategy="custom"时使用)
            exclude_cores: 排除的核心列表

        Returns:
            List[int]: 推荐使用的核心列表
        """
        if not self.is_supported:
            return []

        try:
            cpu_count = psutil.cpu_count(logical=True)
            available_cores = list(range(cpu_count))

            # 排除指定核心
            if exclude_cores:
                available_cores = [c for c in available_cores if c not in exclude_cores]

            if strategy == "custom" and custom_cores:
                # 使用自定义核心列表，但要确保在可用范围内
                return [c for c in custom_cores if c in available_cores]

            elif strategy == "half":
                # 使用前50%的核心
                half_count = max(1, len(available_cores) // 2)
                return available_cores[:half_count]

            else:  # "auto" 默认策略
                # 智能分配：根据CPU核心数采用不同策略
                if cpu_count <= 4:
                    # 低端系统，使用所有核心
                    return available_cores
                elif cpu_count <= 8:
                    # 中端CPU，留一个核心给系统
                    return available_cores[:-1]
                else:
                    # 高端多核CPU，使用前75%的核心
                    use_count = max(1, int(cpu_count * 0.75))
                    return available_cores[:use_count]

        except Exception as e:
            self.logger.error(f"计算最佳核心失败: {e}")
            return []

    def apply_cpu_affinity(self, config: CPUAffinityConfig) -> bool:
        """
        应用CPU亲和性设置

        Args:
            config: CPU亲和性配置

        Returns:
            bool: 是否应用成功
        """
        if not config.enabled or not self.is_supported:
            return False

        try:
            # 保存原始亲和性设置（用于恢复）
            if self.original_affinity is None:
                self.original_affinity = psutil.Process().cpu_affinity()

            # 计算目标核心
            target_cores = self.calculate_optimal_cores(
                strategy=config.strategy,
                custom_cores=config.custom_cores,
                exclude_cores=config.exclude_cores
            )

            if not target_cores:
                self.logger.warning("未找到可用的CPU核心进行绑定")
                return False

            # 应用亲和性设置
            psutil.Process().cpu_affinity(target_cores)

            # 记录成功信息
            sys_info = self.get_system_info()
            self.logger.info(
                f"✅ CPU亲和性设置成功: "
                f"策略={config.strategy}, "
                f"绑定核心={target_cores}, "
                f"系统核心数={sys_info.get('logical_cores', '?')}"
            )
            return True

        except Exception as e:
            self.logger.error(f"❌ CPU亲和性设置失败: {e}")
            return False

    def restore_cpu_affinity(self) -> bool:
        """
        恢复原始CPU亲和性设置

        Returns:
            bool: 是否恢复成功
        """
        if not self.is_supported or self.original_affinity is None:
            return False

        try:
            psutil.Process().cpu_affinity(self.original_affinity)
            self.logger.info(f"🔄 已恢复CPU亲和性设置: {self.original_affinity}")
            return True
        except Exception as e:
            self.logger.error(f"❌ 恢复CPU亲和性失败: {e}")
            return False
```

**3. 增强transcription_service.py**
```python
# backend/app/services/transcription_service.py
"""
转录处理服务 - 整合版本
整合了processor.py和原transcription_service.py的所有功能
"""

import os
import subprocess
import uuid
import threading
import gc
import logging
import shutil
from pathlib import Path
from typing import List, Dict, Optional, Tuple
from pydub import AudioSegment, silence
import whisperx
import torch

from models.job_models import JobSettings, JobState
from services.cpu_affinity_service import CPUAffinityManager, CPUAffinityConfig
from services.hardware_service import get_hardware_detector

# 全局变量（保持向后兼容）
_model_cache: Dict[Tuple[str, str, str], object] = {}
_align_model_cache: Dict[str, Tuple[object, object]] = {}
_model_lock = threading.Lock()
_align_lock = threading.Lock()

# 音频处理配置（后续会移到config.py）
SEGMENT_LEN_MS = 60_000
SILENCE_SEARCH_MS = 2_000
MIN_SILENCE_LEN_MS = 300
SILENCE_THRESH_DBFS = -40

# 进度权重配置
PHASE_WEIGHTS = {
    "extract": 5,      # 音频提取占5%
    "split": 10,       # 音频分段占10%
    "transcribe": 80,  # 转录处理占80%（主要耗时）
    "srt": 5           # SRT生成占5%
}
TOTAL_WEIGHT = sum(PHASE_WEIGHTS.values())


class TranscriptionService:
    """
    转录处理服务
    整合了所有转录相关功能
    """

    def __init__(self, jobs_root: str):
        """
        初始化转录服务

        Args:
            jobs_root: 任务工作目录根路径
        """
        self.jobs_root = Path(jobs_root)
        self.jobs_root.mkdir(parents=True, exist_ok=True)

        self.jobs: Dict[str, JobState] = {}
        self.lock = threading.Lock()
        self.logger = logging.getLogger(__name__)

        # 集成CPU亲和性管理器
        self.cpu_manager = CPUAffinityManager()

        # 集成硬件检测
        self.hardware_detector = get_hardware_detector()
        self._hardware_info = None

        # 记录CPU信息
        sys_info = self.cpu_manager.get_system_info()
        if sys_info.get('supported', False):
            self.logger.info(
                f"💻 CPU信息: {sys_info['logical_cores']}个逻辑核心, "
                f"{sys_info.get('physical_cores', '?')}个物理核心, "
                f"平台: {sys_info.get('platform', '?')}"
            )
        else:
            self.logger.warning("⚠️ CPU亲和性功能不可用")

    def create_job(
        self,
        filename: str,
        src_path: str,
        settings: JobSettings,
        job_id: Optional[str] = None
    ) -> JobState:
        """
        创建转录任务

        Args:
            filename: 文件名
            src_path: 源文件路径
            settings: 任务设置
            job_id: 任务ID（可选，不提供则自动生成）

        Returns:
            JobState: 创建的任务状态对象
        """
        job_id = job_id or uuid.uuid4().hex
        job_dir = self.jobs_root / job_id
        job_dir.mkdir(parents=True, exist_ok=True)

        dest_path = job_dir / filename

        # 复制文件到任务目录
        if os.path.abspath(src_path) != os.path.abspath(dest_path):
            try:
                shutil.copyfile(src_path, dest_path)
                self.logger.debug(f"文件已复制: {src_path} -> {dest_path}")
            except Exception as e:
                self.logger.warning(f"文件复制失败: {e}")

        # 创建任务状态对象
        job = JobState(
            job_id=job_id,
            filename=filename,
            dir=str(job_dir),
            input_path=src_path,
            settings=settings,
            status="uploaded",
            phase="pending",
            message="文件已上传"
        )

        with self.lock:
            self.jobs[job_id] = job

        self.logger.info(f"✅ 任务已创建: {job_id} - {filename}")
        return job

    def get_job(self, job_id: str) -> Optional[JobState]:
        """
        获取任务状态

        Args:
            job_id: 任务ID

        Returns:
            Optional[JobState]: 任务状态对象，不存在则返回None
        """
        with self.lock:
            return self.jobs.get(job_id)

    def start_job(self, job_id: str):
        """
        启动转录任务

        Args:
            job_id: 任务ID
        """
        job = self.get_job(job_id)
        if not job or job.status not in ("uploaded", "failed"):
            self.logger.warning(f"任务无法启动: {job_id}, 状态: {job.status if job else 'not found'}")
            return

        job.canceled = False
        job.error = None
        job.status = "processing"
        job.message = "开始处理"

        # 在独立线程中执行转录
        threading.Thread(
            target=self._run_pipeline,
            args=(job,),
            daemon=True,
            name=f"Transcription-{job_id[:8]}"
        ).start()

        self.logger.info(f"🚀 任务已启动: {job_id}")

    def cancel_job(self, job_id: str) -> bool:
        """
        取消转录任务

        Args:
            job_id: 任务ID

        Returns:
            bool: 是否成功设置取消标志
        """
        job = self.get_job(job_id)
        if not job:
            return False

        job.canceled = True
        job.message = "取消中..."
        self.logger.info(f"🛑 任务取消请求: {job_id}")
        return True

    def _update_progress(
        self,
        job: JobState,
        phase: str,
        phase_ratio: float,
        message: str = ""
    ):
        """
        更新任务进度

        Args:
            job: 任务状态对象
            phase: 当前阶段 (extract/split/transcribe/srt)
            phase_ratio: 当前阶段完成比例 (0.0-1.0)
            message: 进度消息
        """
        job.phase = phase

        # 计算累计进度
        done_weight = 0
        for p, w in PHASE_WEIGHTS.items():
            if p == phase:
                break
            done_weight += w

        current_weight = PHASE_WEIGHTS.get(phase, 0) * max(0.0, min(1.0, phase_ratio))
        job.progress = round((done_weight + current_weight) / TOTAL_WEIGHT * 100, 2)

        if message:
            job.message = message

    def _run_pipeline(self, job: JobState):
        """
        执行转录处理管道

        Args:
            job: 任务状态对象
        """
        # 应用CPU亲和性设置
        cpu_applied = False
        if job.settings.cpu_affinity.enabled:
            cpu_applied = self.cpu_manager.apply_cpu_affinity(
                job.settings.cpu_affinity
            )
            if cpu_applied:
                self.logger.info(f"📌 任务 {job.job_id} 已应用CPU亲和性设置")

        try:
            # 检查取消标志
            if job.canceled:
                job.status = 'canceled'
                job.message = '已取消'
                return

            job_dir = Path(job.dir)
            input_path = job_dir / job.filename
            audio_path = job_dir / 'audio.wav'

            # ========== 阶段1: 提取音频 ==========
            self._update_progress(job, 'extract', 0, '提取音频中')
            if job.canceled:
                raise RuntimeError('任务已取消')

            if not self._extract_audio(str(input_path), str(audio_path)):
                raise RuntimeError('FFmpeg 提取音频失败')

            self._update_progress(job, 'extract', 1, '音频提取完成')
            if job.canceled:
                raise RuntimeError('任务已取消')

            # ========== 阶段2: 智能分段 ==========
            self._update_progress(job, 'split', 0, '音频分段中')
            segments = self._split_audio(str(audio_path))
            if job.canceled:
                raise RuntimeError('任务已取消')

            job.segments = segments
            job.total = len(segments)
            self._update_progress(job, 'split', 1, f'分段完成 共{job.total}段')

            # ========== 阶段3: 转录处理 ==========
            self._update_progress(job, 'transcribe', 0, '加载模型中')
            if job.canceled:
                raise RuntimeError('任务已取消')

            model = self._get_model(job.settings)
            align_cache = {}
            processed_results = []

            for idx, seg in enumerate(segments):
                if job.canceled:
                    raise RuntimeError('任务已取消')

                ratio = idx / max(1, len(segments))
                self._update_progress(
                    job,
                    'transcribe',
                    ratio,
                    f'转录 {idx+1}/{len(segments)}'
                )

                seg_result = self._transcribe_segment(seg, model, job, align_cache)
                if seg_result:
                    processed_results.append(seg_result)

                job.processed = idx + 1

            self._update_progress(job, 'transcribe', 1, '转录完成 生成字幕中')
            if job.canceled:
                raise RuntimeError('任务已取消')

            # ========== 阶段4: 生成SRT ==========
            base_name = os.path.splitext(job.filename)[0]
            srt_path = job_dir / f'{base_name}.srt'
            self._update_progress(job, 'srt', 0, '写入 SRT...')
            self._generate_srt(
                processed_results,
                str(srt_path),
                job.settings.word_timestamps
            )
            self._update_progress(job, 'srt', 1, '处理完成')

            job.srt_path = str(srt_path)

            if job.canceled:
                job.status = 'canceled'
                job.message = '已取消'
            else:
                job.status = 'finished'
                job.message = '完成'
                self.logger.info(f"✅ 任务完成: {job.job_id}")

        except Exception as e:
            if job.canceled and '取消' in str(e):
                job.status = 'canceled'
                job.message = '已取消'
                self.logger.info(f"🛑 任务已取消: {job.job_id}")
            else:
                job.status = 'failed'
                job.message = f'失败: {e}'
                job.error = str(e)
                self.logger.error(f"❌ 任务失败: {job.job_id} - {e}", exc_info=True)

        finally:
            # 恢复CPU亲和性设置
            if cpu_applied:
                restored = self.cpu_manager.restore_cpu_affinity()
                if restored:
                    self.logger.info(f"🔄 任务 {job.job_id} 已恢复CPU亲和性设置")

            # 释放内存
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()

    # ========== 核心处理方法 ==========

    def _extract_audio(self, input_file: str, audio_out: str) -> bool:
        """
        使用FFmpeg提取音频

        Args:
            input_file: 输入文件路径
            audio_out: 输出音频路径

        Returns:
            bool: 是否提取成功
        """
        if os.path.exists(audio_out):
            self.logger.debug(f"音频文件已存在，跳过提取: {audio_out}")
            return True

        # 优先使用项目内的FFmpeg（支持独立打包）
        project_root = Path(__file__).parent.parent.parent
        local_ffmpeg = project_root / "ffmpeg" / "bin" / "ffmpeg.exe"

        if local_ffmpeg.exists():
            ffmpeg_cmd = str(local_ffmpeg)
            self.logger.debug(f"使用项目内FFmpeg: {ffmpeg_cmd}")
        else:
            ffmpeg_cmd = 'ffmpeg'
            self.logger.debug("使用系统FFmpeg")

        cmd = [
            ffmpeg_cmd, '-y', '-i', input_file,
            '-vn',                    # 仅音频
            '-ac', '1',               # 单声道
            '-ar', '16000',           # 16kHz 采样率
            '-acodec', 'pcm_s16le',   # PCM 编码
            audio_out
        ]

        try:
            proc = subprocess.run(
                cmd,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                timeout=600  # 10分钟超时
            )

            if proc.returncode == 0 and os.path.exists(audio_out):
                self.logger.debug(f"✅ 音频提取成功: {audio_out}")
                return True
            else:
                error_msg = proc.stderr.decode('utf-8', errors='ignore')
                self.logger.error(f"❌ FFmpeg执行失败: {error_msg}")
                return False

        except subprocess.TimeoutExpired:
            self.logger.error("❌ FFmpeg超时")
            return False
        except Exception as e:
            self.logger.error(f"❌ 音频提取失败: {e}")
            return False

    def _split_audio(self, audio_path: str) -> List[Dict]:
        """
        智能分段音频（基于静音检测）

        Args:
            audio_path: 音频文件路径

        Returns:
            List[Dict]: 段信息列表，每项包含 file 和 start_ms
        """
        self.logger.debug(f"开始音频分段: {audio_path}")

        audio = AudioSegment.from_wav(audio_path)
        length = len(audio)
        segments = []
        pos = 0
        idx = 0

        while pos < length:
            end = min(pos + SEGMENT_LEN_MS, length)

            # 智能寻找静音点（避免在句子中间分割）
            if end < length and (end - pos) > SILENCE_SEARCH_MS:
                search_start = max(pos, end - SILENCE_SEARCH_MS)
                search_chunk = audio[search_start:end]

                try:
                    silences = silence.detect_silence(
                        search_chunk,
                        min_silence_len=MIN_SILENCE_LEN_MS,
                        silence_thresh=SILENCE_THRESH_DBFS
                    )

                    if silences:
                        # 使用第一个静音点
                        silence_start = silences[0][0]
                        new_end = search_start + silence_start
                        if new_end - pos > MIN_SILENCE_LEN_MS:
                            end = new_end
                except Exception as e:
                    self.logger.warning(f"静音检测失败: {e}")

            # 导出分段
            chunk = audio[pos:end]
            seg_file = os.path.join(os.path.dirname(audio_path), f'segment_{idx}.wav')
            chunk.export(seg_file, format='wav')

            segments.append({
                'file': seg_file,
                'start_ms': pos,
                'duration_ms': end - pos
            })

            pos = end
            idx += 1

        self.logger.debug(f"✅ 音频分段完成: 共{len(segments)}段")
        return segments

    def _get_model(self, settings: JobSettings):
        """
        获取WhisperX模型（带缓存）

        优先使用模型管理器，否则使用简单缓存

        Args:
            settings: 任务设置

        Returns:
            模型对象
        """
        # 尝试使用模型管理器
        try:
            from services.model_preload_manager import get_model_manager
            model_manager = get_model_manager()
            if model_manager:
                return model_manager.get_model(settings)
        except ImportError:
            pass

        # 回退到简单缓存机制
        key = (settings.model, settings.compute_type, settings.device)
        with _model_lock:
            if key in _model_cache:
                self.logger.debug(f"✅ 命中模型缓存: {key}")
                return _model_cache[key]

            self.logger.info(f"🔍 加载模型: {key}")
            m = whisperx.load_model(
                settings.model,
                settings.device,
                compute_type=settings.compute_type
            )
            _model_cache[key] = m
            return m

    def _get_align_model(self, lang: str, device: str):
        """
        获取对齐模型（带缓存）

        Args:
            lang: 语言代码
            device: 设备 (cuda/cpu)

        Returns:
            Tuple[model, metadata]: 对齐模型和元数据
        """
        with _align_lock:
            if lang in _align_model_cache:
                self.logger.debug(f"✅ 命中对齐模型缓存: {lang}")
                return _align_model_cache[lang]

            self.logger.info(f"🔍 加载对齐模型: {lang}")
            am, meta = whisperx.load_align_model(language_code=lang, device=device)
            _align_model_cache[lang] = (am, meta)
            return am, meta

    def _transcribe_segment(
        self,
        seg: Dict,
        model,
        job: JobState,
        align_cache: Dict
    ):
        """
        转录单个音频段

        Args:
            seg: 段信息 {file, start_ms, duration_ms}
            model: Whisper模型
            job: 任务状态
            align_cache: 对齐模型缓存

        Returns:
            Dict: 转录结果（包含segments和word_segments）
        """
        audio = whisperx.load_audio(seg['file'])

        try:
            # Whisper转录
            rs = model.transcribe(
                audio,
                batch_size=job.settings.batch_size,
                verbose=False,
                language=job.language
            )

            if not rs or 'segments' not in rs:
                return None

            # 检测语言
            if not job.language and 'language' in rs:
                job.language = rs['language']
                self.logger.info(f"🌐 检测到语言: {job.language}")

            lang = job.language or rs.get('language')

            # 加载对齐模型
            if lang not in align_cache:
                am, meta = self._get_align_model(lang, job.settings.device)
                align_cache[lang] = (am, meta)

            am, meta = align_cache[lang]

            # 词级对齐
            aligned = whisperx.align(
                rs['segments'],
                am,
                meta,
                audio,
                job.settings.device
            )

            # 时间偏移校正（重要！）
            start_offset = seg['start_ms'] / 1000.0
            final = {'segments': []}

            if 'segments' in aligned:
                for s in aligned['segments']:
                    if 'start' in s:
                        s['start'] += start_offset
                    if 'end' in s:
                        s['end'] += start_offset
                    final['segments'].append(s)

            if 'word_segments' in aligned:
                final['word_segments'] = []
                for w in aligned['word_segments']:
                    if 'start' in w:
                        w['start'] += start_offset
                    if 'end' in w:
                        w['end'] += start_offset
                    final['word_segments'].append(w)

            return final

        finally:
            del audio
            gc.collect()

    def _format_ts(self, sec: float) -> str:
        """
        格式化时间戳为SRT格式

        Args:
            sec: 秒数

        Returns:
            str: SRT时间戳 (HH:MM:SS,mmm)
        """
        if sec < 0:
            sec = 0

        ms = int(round(sec * 1000))
        h = ms // 3600000
        ms %= 3600000
        m = ms // 60000
        ms %= 60000
        s = ms // 1000
        ms %= 1000

        return f"{h:02}:{m:02}:{s:02},{ms:03}"

    def _generate_srt(self, results: List[Dict], path: str, word_level: bool):
        """
        生成SRT字幕文件

        Args:
            results: 转录结果列表
            path: 输出文件路径
            word_level: 是否使用词级时间戳
        """
        lines = []
        n = 1  # 字幕序号

        for r in results:
            if not r:
                continue

            entries = []

            # 词级时间戳模式
            if word_level and r.get('word_segments'):
                for w in r['word_segments']:
                    if w.get('start') is not None and w.get('end') is not None:
                        txt = (w.get('word') or '').strip()
                        if txt:
                            entries.append({
                                'start': w['start'],
                                'end': w['end'],
                                'text': txt
                            })

            # 句子级时间戳模式（默认）
            elif r.get('segments'):
                for s in r['segments']:
                    if s.get('start') is not None and s.get('end') is not None:
                        txt = (s.get('text') or '').strip()
                        if txt:
                            entries.append({
                                'start': s['start'],
                                'end': s['end'],
                                'text': txt
                            })

            # 写入SRT格式
            for e in entries:
                if e['end'] <= e['start']:
                    continue  # 跳过无效时间戳

                lines.append(str(n))  # 序号
                lines.append(
                    f"{self._format_ts(e['start'])} --> {self._format_ts(e['end'])}"
                )  # 时间戳
                lines.append(e['text'])  # 字幕文本
                lines.append("")  # 空行
                n += 1

        # 写入文件
        with open(path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(lines))

        self.logger.info(f"✅ SRT文件已生成: {path}, 共{n-1}条字幕")


# ========== 单例模式（保持向后兼容） ==========

_service_instance: Optional[TranscriptionService] = None

def get_transcription_service(root: str) -> TranscriptionService:
    """
    获取转录服务实例（单例模式）

    Args:
        root: 任务工作目录根路径

    Returns:
        TranscriptionService: 转录服务实例
    """
    global _service_instance
    if _service_instance is None:
        _service_instance = TranscriptionService(root)
    return _service_instance
```

**4. 更新main.py**
```python
# backend/app/main.py

# 修改导入
# from processor import get_processor  # 旧
from services.transcription_service import get_transcription_service  # 新

# 替换全局变量
# proc = get_processor(JOBS_DIR)  # 旧
transcription_service = get_transcription_service(JOBS_DIR)  # 新

# 更新所有API端点
@app.post("/api/create-job")
async def create_job(filename: str = Form(...)):
    try:
        # ... 验证逻辑 ...
        job = transcription_service.create_job(  # 使用新服务
            filename, input_path, JobSettings(), job_id=job_id
        )
        return {"job_id": job_id, "filename": filename}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/start")
async def start(job_id: str = Form(...), settings: str = Form(...)):
    settings_obj = TranscribeSettings(**json.loads(settings))
    job = transcription_service.get_job(job_id)  # 使用新服务
    # ... 其余逻辑 ...
    transcription_service.start_job(job_id)  # 使用新服务
    return {"job_id": job_id, "started": True}

# 更新所有其他API端点...
```

**5. 删除processor.py**
```bash
# 归档processor.py
mv backend/app/processor.py _deprecated/

# 更新说明
cat >> _deprecated/README.md << 'EOF'

- processor.py - 已被services/transcription_service.py替代
  - CPUAffinityManager -> services/cpu_affinity_service.py
  - TranscriptionProcessor -> services/transcription_service.py
EOF
```

**6. 运行黄金标准测试**
```bash
# 启动服务
python backend/app/main.py

# 运行测试视频
# 上传 tests/fixtures/test_video.mp4
# 下载生成的字幕到 output/new.srt

# 对比结果
python tests/test_golden_master.py
```

**7. 提交变更**
```bash
git add .
git commit -m "refactor: 合并转录逻辑，统一到transcription_service"
```

**完成标准**:
- ✅ 只有一个转录服务实现
- ✅ CPU亲和性功能正常
- ✅ 所有API正常工作
- ✅ 黄金标准测试通过
- ✅ processor.py已删除

---

### 阶段2: 结构整理 (Day 2)

#### 步骤2.1: 提取配置管理 (2小时)

**目标**: 消除硬编码，支持独立打包

**创建统一配置**:

```python
# backend/app/core/config.py
"""
统一配置管理
严格遵守独立打包原则：
1. 杜绝硬编码绝对路径
2. 杜绝依赖系统环境变量
3. 强制接管模型下载路径
"""

import os
from pathlib import Path
from typing import Optional

class ProjectConfig:
    """项目配置类"""

    def __init__(self):
        # ========== 路径配置（基于项目根目录） ==========
        # 获取项目根目录（从当前文件位置向上三级）
        self.BASE_DIR = Path(__file__).parent.parent.parent.parent.resolve()

        # 输入输出目录
        self.INPUT_DIR = self.BASE_DIR / "input"
        self.OUTPUT_DIR = self.BASE_DIR / "output"
        self.JOBS_DIR = self.BASE_DIR / "jobs"
        self.TEMP_DIR = self.BASE_DIR / "temp"

        # FFmpeg路径（优先使用项目内的，支持独立打包）
        self.FFMPEG_DIR = self.BASE_DIR / "ffmpeg" / "bin"
        self.FFMPEG_EXE = self.FFMPEG_DIR / "ffmpeg.exe"

        # 模型缓存目录（强制接管，不使用默认的用户目录）
        self.MODELS_DIR = self.BASE_DIR / "models"
        self.HF_CACHE_DIR = self.MODELS_DIR / "huggingface"
        self.TORCH_CACHE_DIR = self.MODELS_DIR / "torch"

        # 设置环境变量，强制模型下载到项目目录
        os.environ['HF_HOME'] = str(self.HF_CACHE_DIR)
        os.environ['TORCH_HOME'] = str(self.TORCH_CACHE_DIR)
        os.environ['TRANSFORMERS_CACHE'] = str(self.HF_CACHE_DIR / "transformers")

        # 确保目录存在
        for dir_path in [
            self.INPUT_DIR,
            self.OUTPUT_DIR,
            self.JOBS_DIR,
            self.TEMP_DIR,
            self.MODELS_DIR,
            self.HF_CACHE_DIR,
            self.TORCH_CACHE_DIR,
            self.FFMPEG_DIR
        ]:
            dir_path.mkdir(parents=True, exist_ok=True)

        # ========== 音频处理配置 ==========
        self.SEGMENT_LENGTH_MS = 60_000      # 60秒
        self.SILENCE_SEARCH_MS = 2_000       # 2秒
        self.MIN_SILENCE_LEN_MS = 300        # 300毫秒
        self.SILENCE_THRESHOLD_DBFS = -40    # -40dB

        # ========== 模型配置 ==========
        self.DEFAULT_MODEL = "medium"
        self.DEFAULT_DEVICE = "cuda"  # 自动检测会覆盖
        self.DEFAULT_COMPUTE_TYPE = "float16"
        self.DEFAULT_BATCH_SIZE = 16
        self.MAX_CACHE_SIZE = 3              # 最多缓存3个模型
        self.MEMORY_THRESHOLD = 0.8          # 内存使用阈值

        # ========== 服务器配置 ==========
        self.API_HOST = "127.0.0.1"
        self.API_PORT = 8000
        self.API_RELOAD = False

        # ========== CPU亲和性配置 ==========
        self.CPU_AFFINITY_ENABLED = True
        self.CPU_AFFINITY_STRATEGY = "auto"  # auto/half/custom

        # ========== 日志配置 ==========
        self.LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO")
        self.LOG_FILE = self.BASE_DIR / "logs" / "app.log"
        self.LOG_FILE.parent.mkdir(parents=True, exist_ok=True)

    def get_ffmpeg_command(self) -> str:
        """
        获取FFmpeg命令
        优先使用项目内的FFmpeg，支持独立打包

        Returns:
            str: FFmpeg可执行文件路径
        """
        if self.FFMPEG_EXE.exists():
            # 使用项目内的FFmpeg
            return str(self.FFMPEG_EXE)
        else:
            # 回退到系统命令
            return "ffmpeg"

    def get_audio_config(self) -> dict:
        """获取音频处理配置"""
        return {
            "segment_length_ms": self.SEGMENT_LENGTH_MS,
            "silence_search_ms": self.SILENCE_SEARCH_MS,
            "min_silence_len_ms": self.MIN_SILENCE_LEN_MS,
            "silence_threshold_dbfs": self.SILENCE_THRESHOLD_DBFS
        }

    def get_model_config(self) -> dict:
        """获取模型配置"""
        return {
            "default_model": self.DEFAULT_MODEL,
            "default_device": self.DEFAULT_DEVICE,
            "default_compute_type": self.DEFAULT_COMPUTE_TYPE,
            "default_batch_size": self.DEFAULT_BATCH_SIZE,
            "max_cache_size": self.MAX_CACHE_SIZE,
            "memory_threshold": self.MEMORY_THRESHOLD
        }

# 全局配置实例
config = ProjectConfig()

# 打印配置信息（启动时显示）
print(f"""
🔧 项目配置已加载
📁 项目根目录: {config.BASE_DIR}
📥 输入目录: {config.INPUT_DIR}
📤 输出目录: {config.OUTPUT_DIR}
🎬 FFmpeg: {config.get_ffmpeg_command()}
🤖 模型缓存: {config.MODELS_DIR}
""")
```

**更新transcription_service.py使用配置**:

```python
# backend/app/services/transcription_service.py

from core.config import config

class TranscriptionService:
    def _extract_audio(self, input_file: str, audio_out: str) -> bool:
        """使用配置中的FFmpeg"""
        ffmpeg_cmd = config.get_ffmpeg_command()  # 使用配置

        cmd = [
            ffmpeg_cmd, '-y', '-i', input_file,
            # ... 其余参数
        ]
        # ... 其余逻辑

    def _split_audio(self, audio_path: str) -> List[Dict]:
        """使用配置中的音频参数"""
        audio_config = config.get_audio_config()

        SEGMENT_LEN_MS = audio_config['segment_length_ms']
        SILENCE_SEARCH_MS = audio_config['silence_search_ms']
        # ... 使用配置参数
```

**更新main.py使用配置**:

```python
# backend/app/main.py

from core.config import config

# 使用配置中的路径
INPUT_DIR = config.INPUT_DIR
OUTPUT_DIR = config.OUTPUT_DIR
JOBS_DIR = config.JOBS_DIR

# ... 其余逻辑
```

**提交变更**:
```bash
git add .
git commit -m "refactor: 提取配置管理，支持独立打包"
```

---

#### 步骤2.2: 简化日志配置 (1小时)

**创建简单日志配置**:

```python
# backend/app/core/logging.py
"""
简单的日志配置
"""

import logging
import sys
from pathlib import Path
from core.config import config

def setup_logging():
    """配置日志系统"""

    # 配置格式
    log_format = '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
    date_format = '%Y-%m-%d %H:%M:%S'

    # 配置日志
    logging.basicConfig(
        level=getattr(logging, config.LOG_LEVEL),
        format=log_format,
        datefmt=date_format,
        handlers=[
            # 控制台输出
            logging.StreamHandler(sys.stdout),
            # 文件输出
            logging.FileHandler(config.LOG_FILE, encoding='utf-8')
        ]
    )

    # 设置第三方库日志级别
    logging.getLogger('urllib3').setLevel(logging.WARNING)
    logging.getLogger('multipart').setLevel(logging.WARNING)

    logger = logging.getLogger(__name__)
    logger.info(f"📝 日志系统已初始化 - 级别: {config.LOG_LEVEL}")
    logger.info(f"📄 日志文件: {config.LOG_FILE}")

    return logger
```

**在main.py中使用**:

```python
# backend/app/main.py

from core.logging import setup_logging

# 在应用启动时初始化日志
logger = setup_logging()

@app.on_event("startup")
async def startup_event():
    logger.info("🚀 应用启动中...")
    # ... 其余启动逻辑
```

---

#### 步骤2.3: 整理目录结构 (1小时)

**目标结构**:
```
backend/app/
├── main.py                  # 唯一入口
├── core/                    # 核心层
│   ├── __init__.py
│   ├── config.py            # 配置管理
│   └── logging.py           # 日志配置
├── services/                # 服务层
│   ├── __init__.py
│   ├── transcription_service.py
│   ├── cpu_affinity_service.py
│   ├── hardware_service.py
│   ├── file_service.py
│   └── model_preload_manager.py
├── api/                     # API层
│   ├── __init__.py
│   └── routes/
│       ├── __init__.py
│       ├── transcription_routes.py
│       ├── file_routes.py
│       └── hardware_routes.py
└── models/                  # 数据模型
    ├── __init__.py
    ├── job_models.py
    └── hardware_models.py
```

**创建__init__.py**:
```python
# backend/app/core/__init__.py
from .config import config
from .logging import setup_logging

__all__ = ['config', 'setup_logging']
```

**运行黄金标准测试验证**:
```bash
python tests/test_golden_master.py
```

**提交变更**:
```bash
git add .
git commit -m "refactor: 整理目录结构，清晰分层"
```

**完成标准**:
- ✅ 配置统一管理
- ✅ 路径使用相对路径
- ✅ 日志系统简单有效
- ✅ 目录结构清晰
- ✅ 黄金标准测试通过

---

### 阶段2.5: 模型与数据集管理器 (Day 2.5-3, 预计2-3小时)

#### 目标
- 统一管理Whisper模型和对齐模型的生命周期
- 支持前端图形化下载、删除模型
- 自动检测语言并下载所需模型
- 为断点续传提供模型自动补齐能力

#### 步骤2.5.1: 创建模型管理服务 (1小时)

**创建服务**:
```python
# backend/app/services/model_manager_service.py
"""
统一模型与数据集管理服务
- 下载管理
- 缓存管理
- 自动检测语言并下载
- 版本管理
"""

from dataclasses import dataclass
from typing import List, Optional, Dict
from pathlib import Path
import threading
import logging

@dataclass
class ModelInfo:
    """模型信息"""
    model_id: str           # 例如: "large-v2"
    size_mb: int            # 模型大小(MB)
    status: str             # "not_downloaded", "downloading", "ready", "error"
    download_progress: float  # 0-100
    local_path: Optional[Path] = None
    description: str = ""

@dataclass
class AlignModelInfo:
    """对齐模型信息"""
    language: str           # 例如: "zh", "en"
    language_name: str      # 例如: "中文", "English"
    status: str
    download_progress: float
    local_path: Optional[Path] = None

class ModelManagerService:
    """
    模型管理服务
    统一管理Whisper模型和对齐模型的下载、缓存、删除
    """
    
    # 支持的Whisper模型
    WHISPER_MODELS = {
        "tiny": {"size_mb": 75, "desc": "最快，精度较低"},
        "base": {"size_mb": 145, "desc": "快速，精度一般"},
        "small": {"size_mb": 490, "desc": "平衡速度与精度"},
        "medium": {"size_mb": 1500, "desc": "较慢，精度较高"},
        "large-v2": {"size_mb": 3100, "desc": "最慢，精度最高"},
    }
    
    # 支持的语言（对齐模型）
    SUPPORTED_LANGUAGES = {
        "zh": "中文 (Chinese)",
        "en": "英语 (English)",
        "ja": "日语 (Japanese)",
        "ko": "韩语 (Korean)",
        "es": "西班牙语 (Spanish)",
        "fr": "法语 (French)",
        "de": "德语 (German)",
        # ... 更多语言
    }
    
    def __init__(self, models_dir: Path):
        self.models_dir = models_dir
        self.logger = logging.getLogger(__name__)
        
        # 模型状态跟踪
        self.whisper_models: Dict[str, ModelInfo] = {}
        self.align_models: Dict[str, AlignModelInfo] = {}
        
        # 下载队列和锁
        self.download_queue = []
        self.download_lock = threading.Lock()
        
        # 初始化模型信息
        self._init_model_info()
    
    def _init_model_info(self):
        """扫描本地已有模型"""
        for model_id, info in self.WHISPER_MODELS.items():
            status = self._check_whisper_model_exists(model_id)
            self.whisper_models[model_id] = ModelInfo(
                model_id=model_id,
                size_mb=info["size_mb"],
                status=status,
                download_progress=100.0 if status == "ready" else 0.0,
                description=info["desc"]
            )
        
        for lang, name in self.SUPPORTED_LANGUAGES.items():
            status = self._check_align_model_exists(lang)
            self.align_models[lang] = AlignModelInfo(
                language=lang,
                language_name=name,
                status=status,
                download_progress=100.0 if status == "ready" else 0.0
            )
    
    def list_whisper_models(self) -> List[ModelInfo]:
        """列出所有Whisper模型状态"""
        return list(self.whisper_models.values())
    
    def list_align_models(self) -> List[AlignModelInfo]:
        """列出所有对齐模型状态"""
        return list(self.align_models.values())
    
    def download_whisper_model(self, model_id: str) -> bool:
        """
        下载Whisper模型
        
        Args:
            model_id: 模型ID
            
        Returns:
            bool: 是否成功加入下载队列
        """
        if model_id not in self.whisper_models:
            return False
        
        model = self.whisper_models[model_id]
        if model.status == "downloading":
            return False  # 已在下载中
        
        # 加入下载队列
        model.status = "downloading"
        model.download_progress = 0.0
        
        threading.Thread(
            target=self._download_whisper_model_task,
            args=(model_id,),
            daemon=True
        ).start()
        
        return True
    
    def download_align_model(self, language: str) -> bool:
        """下载对齐模型"""
        if language not in self.align_models:
            return False
        
        model = self.align_models[language]
        if model.status == "downloading":
            return False
        
        model.status = "downloading"
        model.download_progress = 0.0
        
        threading.Thread(
            target=self._download_align_model_task,
            args=(language,),
            daemon=True
        ).start()
        
        return True
    
    def auto_download_for_language(self, language: str) -> bool:
        """
        自动下载指定语言所需的对齐模型
        用于断点续传恢复时自动补齐模型
        
        Args:
            language: 语言代码
            
        Returns:
            bool: 是否需要下载（True）或已存在（False）
        """
        if language not in self.align_models:
            self.logger.warning(f"不支持的语言: {language}")
            return False
        
        model = self.align_models[language]
        
        if model.status == "ready":
            self.logger.info(f"✅ 对齐模型已存在: {language}")
            return False
        
        self.logger.info(f"🔍 检测到新语言 {language}，开始自动下载对齐模型")
        return self.download_align_model(language)
    
    def delete_whisper_model(self, model_id: str) -> bool:
        """删除Whisper模型"""
        # 实现删除逻辑
        pass
    
    def delete_align_model(self, language: str) -> bool:
        """删除对齐模型"""
        # 实现删除逻辑
        pass
    
    def get_download_progress(self) -> Dict:
        """获取所有下载进度"""
        return {
            "whisper": {
                mid: {
                    "status": m.status,
                    "progress": m.download_progress
                }
                for mid, m in self.whisper_models.items()
            },
            "align": {
                lang: {
                    "status": m.status,
                    "progress": m.download_progress
                }
                for lang, m in self.align_models.items()
            }
        }
    
    def _download_whisper_model_task(self, model_id: str):
        """下载任务（后台线程）"""
        try:
            model = self.whisper_models[model_id]
            
            # 使用whisperx的下载接口，带进度回调
            import whisperx
            
            def progress_callback(current, total):
                model.download_progress = (current / total) * 100
            
            # 下载模型
            whisperx.load_model(
                model_id,
                device="cpu",  # 仅下载，不加载到GPU
                download_root=str(self.models_dir / "whisper"),
                # 需要修改whisperx源码支持进度回调
            )
            
            model.status = "ready"
            model.download_progress = 100.0
            self.logger.info(f"✅ Whisper模型下载完成: {model_id}")
            
        except Exception as e:
            model.status = "error"
            self.logger.error(f"❌ Whisper模型下载失败: {model_id} - {e}")
    
    def _download_align_model_task(self, language: str):
        """下载对齐模型任务"""
        try:
            model = self.align_models[language]
            
            import whisperx
            
            def progress_callback(current, total):
                model.download_progress = (current / total) * 100
            
            # 下载对齐模型
            whisperx.load_align_model(
                language_code=language,
                device="cpu",
                model_dir=str(self.models_dir / "align"),
            )
            
            model.status = "ready"
            model.download_progress = 100.0
            self.logger.info(f"✅ 对齐模型下载完成: {language}")
            
        except Exception as e:
            model.status = "error"
            self.logger.error(f"❌ 对齐模型下载失败: {language} - {e}")
    
    # ... 其他辅助方法
```

**功能清单**:
- ✅ 扫描本地已有模型
- ✅ 列出所有支持的模型
- ✅ 下载模型（带进度）
- ✅ 删除模型
- ✅ 自动检测语言并下载

#### 步骤2.5.2: 创建API端点 (30分钟)

**创建路由**:
```python
# backend/app/api/routes/model_routes.py
"""
模型管理API路由
"""

from fastapi import APIRouter, HTTPException
from typing import List
from models.model_models import ModelInfo, AlignModelInfo

router = APIRouter(prefix="/api/models", tags=["models"])

@router.get("/whisper", response_model=List[ModelInfo])
async def list_whisper_models():
    """列出所有Whisper模型"""
    return model_manager.list_whisper_models()

@router.get("/align", response_model=List[AlignModelInfo])
async def list_align_models():
    """列出所有对齐模型"""
    return model_manager.list_align_models()

@router.post("/whisper/{model_id}/download")
async def download_whisper_model(model_id: str):
    """下载指定的Whisper模型"""
    success = model_manager.download_whisper_model(model_id)
    if not success:
        raise HTTPException(400, "模型不存在或正在下载中")
    return {"success": True, "message": f"开始下载模型 {model_id}"}

@router.post("/align/{language}/download")
async def download_align_model(language: str):
    """下载指定语言的对齐模型"""
    success = model_manager.download_align_model(language)
    if not success:
        raise HTTPException(400, "语言不支持或正在下载中")
    return {"success": True, "message": f"开始下载 {language} 对齐模型"}

@router.delete("/whisper/{model_id}")
async def delete_whisper_model(model_id: str):
    """删除指定的Whisper模型"""
    success = model_manager.delete_whisper_model(model_id)
    if not success:
        raise HTTPException(400, "删除失败")
    return {"success": True}

@router.delete("/align/{language}")
async def delete_align_model(language: str):
    """删除指定的对齐模型"""
    success = model_manager.delete_align_model(language)
    if not success:
        raise HTTPException(400, "删除失败")
    return {"success": True}

@router.get("/progress")
async def get_download_progress():
    """获取所有下载进度"""
    return model_manager.get_download_progress()
```

**在main.py中注册**:
```python
from api.routes import model_routes
app.include_router(model_routes.router)
```

#### 步骤2.5.3: 创建前端管理界面 (1小时)

**创建组件**:
```vue
<!-- frontend/src/components/ModelManager.vue -->
<template>
  <div class="model-manager">
    <h2>🤖 模型管理</h2>
    
    <!-- Whisper模型列表 -->
    <div class="model-section">
      <h3>Whisper模型</h3>
      <div class="model-grid">
        <div 
          v-for="model in whisperModels" 
          :key="model.model_id"
          class="model-card"
        >
          <div class="model-header">
            <span class="model-name">{{ model.model_id }}</span>
            <span class="model-size">{{ model.size_mb }}MB</span>
          </div>
          
          <div class="model-desc">{{ model.description }}</div>
          
          <div class="model-status">
            <!-- 未下载 -->
            <button 
              v-if="model.status === 'not_downloaded'"
              @click="downloadWhisperModel(model.model_id)"
              class="btn-download"
            >
              ⬇️ 下载
            </button>
            
            <!-- 下载中 -->
            <div v-else-if="model.status === 'downloading'" class="progress-bar">
              <div 
                class="progress-fill" 
                :style="{ width: model.download_progress + '%' }"
              ></div>
              <span class="progress-text">{{ model.download_progress.toFixed(1) }}%</span>
            </div>
            
            <!-- 已下载 -->
            <div v-else-if="model.status === 'ready'" class="status-ready">
              ✅ 已安装
              <button @click="deleteWhisperModel(model.model_id)" class="btn-delete">
                🗑️ 删除
              </button>
            </div>
            
            <!-- 错误 -->
            <div v-else-if="model.status === 'error'" class="status-error">
              ❌ 下载失败
              <button @click="downloadWhisperModel(model.model_id)" class="btn-retry">
                🔄 重试
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
    
    <!-- 对齐模型列表 -->
    <div class="model-section">
      <h3>语言对齐模型</h3>
      <div class="model-grid">
        <div 
          v-for="model in alignModels" 
          :key="model.language"
          class="model-card"
        >
          <div class="model-header">
            <span class="model-name">{{ model.language_name }}</span>
          </div>
          
          <div class="model-status">
            <!-- 类似Whisper模型的状态显示 -->
            <!-- ... -->
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted, onUnmounted } from 'vue'
import axios from 'axios'

const whisperModels = ref([])
const alignModels = ref([])
let progressInterval = null

// 加载模型列表
async function loadModels() {
  try {
    const [whisperRes, alignRes] = await Promise.all([
      axios.get('/api/models/whisper'),
      axios.get('/api/models/align')
    ])
    
    whisperModels.value = whisperRes.data
    alignModels.value = alignRes.data
  } catch (error) {
    console.error('加载模型列表失败:', error)
  }
}

// 下载Whisper模型
async function downloadWhisperModel(modelId) {
  try {
    await axios.post(`/api/models/whisper/${modelId}/download`)
    // 开始轮询进度
    startProgressPolling()
  } catch (error) {
    console.error('下载失败:', error)
  }
}

// 轮询下载进度
function startProgressPolling() {
  if (progressInterval) return
  
  progressInterval = setInterval(async () => {
    try {
      const res = await axios.get('/api/models/progress')
      updateModelsProgress(res.data)
      
      // 检查是否所有下载完成
      const allComplete = checkAllDownloadsComplete()
      if (allComplete) {
        stopProgressPolling()
      }
    } catch (error) {
      console.error('获取进度失败:', error)
    }
  }, 1000)
}

function stopProgressPolling() {
  if (progressInterval) {
    clearInterval(progressInterval)
    progressInterval = null
  }
}

// 更新模型进度
function updateModelsProgress(progressData) {
  // 更新Whisper模型
  Object.entries(progressData.whisper).forEach(([modelId, data]) => {
    const model = whisperModels.value.find(m => m.model_id === modelId)
    if (model) {
      model.status = data.status
      model.download_progress = data.progress
    }
  })
  
  // 更新对齐模型
  Object.entries(progressData.align).forEach(([lang, data]) => {
    const model = alignModels.value.find(m => m.language === lang)
    if (model) {
      model.status = data.status
      model.download_progress = data.progress
    }
  })
}

// ... 其他方法

onMounted(() => {
  loadModels()
  startProgressPolling()
})

onUnmounted(() => {
  stopProgressPolling()
})
</script>

<style scoped>
.model-manager {
  padding: 20px;
}

.model-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));
  gap: 20px;
  margin-top: 10px;
}

.model-card {
  border: 1px solid #ddd;
  border-radius: 8px;
  padding: 15px;
  background: white;
}

.progress-bar {
  position: relative;
  height: 30px;
  background: #f0f0f0;
  border-radius: 4px;
  overflow: hidden;
}

.progress-fill {
  height: 100%;
  background: linear-gradient(90deg, #4caf50, #8bc34a);
  transition: width 0.3s ease;
}

.progress-text {
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  font-weight: bold;
}

/* ... 其他样式 */
</style>
```

**集成到主界面**:
```vue
<!-- frontend/src/App.vue -->
<template>
  <div>
    <TabBar v-model="activeTab" />
    
    <TranscriptionView v-if="activeTab === 'transcription'" />
    <ModelManager v-else-if="activeTab === 'models'" />
  </div>
</template>
```

#### 步骤2.5.4: 集成到转录服务 (30分钟)

**修改transcription_service.py**:
- 在`_get_align_model()`中添加自动下载逻辑
- 检测到新语言时触发下载
- 等待下载完成后继续转录
```python
# backend/app/services/transcription_service.py

class TranscriptionService:
    def __init__(self, jobs_root: str):
        # ... 原有初始化 ...
        
        # 集成模型管理器
        from services.model_manager_service import get_model_manager
        self.model_manager = get_model_manager()
    
    def _get_align_model(self, lang: str, device: str):
        """
        获取对齐模型（自动下载）
        
        修改点：检测到新语言时，自动触发下载
        """
        # 检查模型是否存在
        if not self._align_model_exists(lang):
            self.logger.info(f"🔍 检测到新语言: {lang}")
            
            # 自动下载
            download_started = self.model_manager.auto_download_for_language(lang)
            
            if download_started:
                # 等待下载完成（带超时）
                self._wait_for_model_download(lang, timeout=300)
        
        # 加载模型
        with _align_lock:
            if lang in _align_model_cache:
                return _align_model_cache[lang]
            
            am, meta = whisperx.load_align_model(language_code=lang, device=device)
            _align_model_cache[lang] = (am, meta)
            return am, meta
    
    def _wait_for_model_download(self, language: str, timeout: int = 300):
        """
        等待模型下载完成
        
        Args:
            language: 语言代码
            timeout: 超时时间（秒）
        """
        import time
        start_time = time.time()
        
        while time.time() - start_time < timeout:
            model_info = self.model_manager.align_models.get(language)
            
            if model_info and model_info.status == "ready":
                self.logger.info(f"✅ 模型下载完成: {language}")
                return True
            elif model_info and model_info.status == "error":
                raise RuntimeError(f"模型下载失败: {language}")
            
            time.sleep(2)  # 每2秒检查一次
        
        raise TimeoutError(f"模型下载超时: {language}")
```


**完成标准**:
- ✅ 前端可以查看所有模型状态
- ✅ 可以手动下载/删除模型
- ✅ 下载进度实时显示
- ✅ 转录时自动下载缺失的模型
- ✅ 断点续传恢复时自动补齐模型


### 阶段3: 实现断点续传 (Day 3)

#### 步骤3.1: 设计检查点结构 (30分钟)

**检查点数据结构**:
```python
# backend/app/models/checkpoint_models.py
"""
检查点数据模型
"""

from dataclasses import dataclass, asdict
from typing import List, Dict, Optional
from datetime import datetime
import json

@dataclass
class Checkpoint:
    """
    转录任务检查点
    用于断点续传
    """
    job_id: str
    phase: str  # extract/split/transcribe/srt
    progress: float

    # 音频处理信息
    audio_path: Optional[str] = None
    segments: List[Dict] = None  # 所有段信息

    # 转录进度
    total_segments: int = 0
    processed_segments: List[int] = None  # 已处理的段索引
    results: List[Dict] = None  # 已转录的结果

    # 语言检测
    language: Optional[str] = None

    # 时间戳
    created_at: str = None
    updated_at: str = None

    def __post_init__(self):
        """初始化默认值"""
        if self.segments is None:
            self.segments = []
        if self.processed_segments is None:
            self.processed_segments = []
        if self.results is None:
            self.results = []
        if self.created_at is None:
            self.created_at = datetime.now().isoformat()
        if self.updated_at is None:
            self.updated_at = datetime.now().isoformat()

    def to_dict(self) -> dict:
        """转换为字典"""
        return asdict(self)

    def to_json(self) -> str:
        """转换为JSON字符串"""
        return json.dumps(self.to_dict(), ensure_ascii=False, indent=2)

    @classmethod
    def from_dict(cls, data: dict) -> 'Checkpoint':
        """从字典创建"""
        return cls(**data)

    @classmethod
    def from_json(cls, json_str: str) -> 'Checkpoint':
        """从JSON字符串创建"""
        return cls.from_dict(json.loads(json_str))
```

---

#### 步骤3.2: 实现检查点服务 (2小时)

```python
# backend/app/services/checkpoint_service.py
"""
检查点管理服务
实现断点续传功能
"""

import logging
from pathlib import Path
from typing import Optional
from models.checkpoint_models import Checkpoint

class CheckpointService:
    """检查点管理服务"""

    def __init__(self, jobs_root: Path):
        """
        初始化检查点服务

        Args:
            jobs_root: 任务根目录
        """
        self.jobs_root = Path(jobs_root)
        self.logger = logging.getLogger(__name__)

    def _get_checkpoint_path(self, job_id: str) -> Path:
        """
        获取检查点文件路径

        Args:
            job_id: 任务ID

        Returns:
            Path: 检查点文件路径
        """
        return self.jobs_root / job_id / "checkpoint.json"

    def save(self, checkpoint: Checkpoint):
        """
        保存检查点

        Args:
            checkpoint: 检查点对象
        """
        checkpoint_file = self._get_checkpoint_path(checkpoint.job_id)
        checkpoint_file.parent.mkdir(parents=True, exist_ok=True)

        # 更新时间戳
        from datetime import datetime
        checkpoint.updated_at = datetime.now().isoformat()

        # 写入文件
        with open(checkpoint_file, 'w', encoding='utf-8') as f:
            f.write(checkpoint.to_json())

        self.logger.debug(
            f"💾 检查点已保存: {checkpoint.job_id} - "
            f"{checkpoint.phase} - {checkpoint.progress:.1f}%"
        )

    def load(self, job_id: str) -> Optional[Checkpoint]:
        """
        加载检查点

        Args:
            job_id: 任务ID

        Returns:
            Optional[Checkpoint]: 检查点对象，不存在则返回None
        """
        checkpoint_file = self._get_checkpoint_path(job_id)

        if not checkpoint_file.exists():
            return None

        try:
            with open(checkpoint_file, 'r', encoding='utf-8') as f:
                json_str = f.read()

            checkpoint = Checkpoint.from_json(json_str)
            self.logger.info(
                f"📂 检查点已加载: {job_id} - "
                f"{checkpoint.phase} - {checkpoint.progress:.1f}%"
            )
            return checkpoint

        except Exception as e:
            self.logger.error(f"❌ 检查点加载失败: {job_id} - {e}")
            return None

    def exists(self, job_id: str) -> bool:
        """
        检查检查点是否存在

        Args:
            job_id: 任务ID

        Returns:
            bool: 是否存在
        """
        return self._get_checkpoint_path(job_id).exists()

    def delete(self, job_id: str):
        """
        删除检查点

        Args:
            job_id: 任务ID
        """
        checkpoint_file = self._get_checkpoint_path(job_id)
        if checkpoint_file.exists():
            checkpoint_file.unlink()
            self.logger.debug(f"🗑️ 检查点已删除: {job_id}")
```

---

#### 步骤3.3: 集成断点续传到转录服务 (3小时)

```python
# backend/app/services/transcription_service.py

from services.checkpoint_service import CheckpointService
from models.checkpoint_models import Checkpoint

class TranscriptionService:
    """转录服务（支持断点续传）"""

    def __init__(self, jobs_root: str):
        # ... 原有初始化 ...

        # 添加检查点服务
        self.checkpoint_service = CheckpointService(self.jobs_root)

    def _run_pipeline(self, job: JobState):
        """转录流程（支持断点续传）"""

        # ... 原有CPU亲和性设置 ...

        try:
            # ========== 尝试加载检查点 ==========
            checkpoint = self.checkpoint_service.load(job.job_id)

            if checkpoint:
                self.logger.info(f"🔄 从检查点恢复任务: {job.job_id}")
                self._resume_from_checkpoint(job, checkpoint)
            else:
                self.logger.info(f"🚀 开始新任务: {job.job_id}")
                self._start_new_transcription(job)

        except Exception as e:
            # 保存失败状态的检查点
            self.checkpoint_service.save(Checkpoint(
                job_id=job.job_id,
                phase=job.phase,
                progress=job.progress
            ))
            raise

        finally:
            # ... 原有清理逻辑 ...
            pass

    def _start_new_transcription(self, job: JobState):
        """开始新的转录任务"""
        job_dir = Path(job.dir)
        input_path = job_dir / job.filename
        audio_path = job_dir / 'audio.wav'

        # ========== 阶段1: 提取音频 ==========
        self._update_progress(job, 'extract', 0, '提取音频中')
        if job.canceled:
            raise RuntimeError('任务已取消')

        if not self._extract_audio(str(input_path), str(audio_path)):
            raise RuntimeError('FFmpeg 提取音频失败')

        # 保存检查点
        self.checkpoint_service.save(Checkpoint(
            job_id=job.job_id,
            phase='extract',
            progress=5.0,
            audio_path=str(audio_path)
        ))

        self._update_progress(job, 'extract', 1, '音频提取完成')

        # ========== 阶段2: 智能分段 ==========
        self._update_progress(job, 'split', 0, '音频分段中')
        if job.canceled:
            raise RuntimeError('任务已取消')

        segments = self._split_audio(str(audio_path))
        job.segments = segments
        job.total = len(segments)

        # 保存检查点
        self.checkpoint_service.save(Checkpoint(
            job_id=job.job_id,
            phase='split',
            progress=15.0,
            audio_path=str(audio_path),
            segments=[
                {"file": str(seg['file']), "start_ms": seg['start_ms']}
                for seg in segments
            ],
            total_segments=len(segments)
        ))

        self._update_progress(job, 'split', 1, f'分段完成 共{job.total}段')

        # ========== 阶段3: 转录处理 ==========
        self._transcribe_all_segments(job, segments)

    def _resume_from_checkpoint(self, job: JobState, checkpoint: Checkpoint):
        """从检查点恢复任务"""
        phase = checkpoint.phase

        if phase == 'extract':
            # 音频已提取，继续分段
            self.logger.info("📍 恢复点: 音频已提取，开始分段")
            audio_path = Path(checkpoint.audio_path)

            segments = self._split_audio(str(audio_path))
            job.segments = segments
            job.total = len(segments)

            # 更新检查点
            checkpoint.phase = 'split'
            checkpoint.progress = 15.0
            checkpoint.segments = [
                {"file": str(seg['file']), "start_ms": seg['start_ms']}
                for seg in segments
            ]
            checkpoint.total_segments = len(segments)
            self.checkpoint_service.save(checkpoint)

            self._transcribe_all_segments(job, segments)

        elif phase == 'split':
            # 分段已完成，开始转录
            self.logger.info("📍 恢复点: 分段已完成，开始转录")
            segments = [
                {"file": Path(seg['file']), "start_ms": seg['start_ms']}
                for seg in checkpoint.segments
            ]
            job.segments = segments
            job.total = len(segments)

            self._transcribe_all_segments(job, segments)

        elif phase == 'transcribe':
            # 部分转录已完成，继续未完成的部分
            processed_count = len(checkpoint.processed_segments)
            total_count = checkpoint.total_segments
            self.logger.info(
                f"📍 恢复点: 继续转录 "
                f"(已完成 {processed_count}/{total_count} 段)"
            )

            segments = [
                {"file": Path(seg['file']), "start_ms": seg['start_ms']}
                for seg in checkpoint.segments
            ]
            job.segments = segments
            job.total = len(segments)
            job.language = checkpoint.language

            # 继续未完成的段
            self._transcribe_all_segments(
                job,
                segments,
                skip_indices=set(checkpoint.processed_segments),
                existing_results=checkpoint.results
            )

        elif phase == 'srt':
            # 转录已完成，重新生成SRT
            self.logger.info("📍 恢复点: 重新生成SRT")
            job.language = checkpoint.language

            base_name = os.path.splitext(job.filename)[0]
            srt_path = Path(job.dir) / f'{base_name}.srt'

            self._generate_srt(
                checkpoint.results,
                str(srt_path),
                job.settings.word_timestamps
            )

            job.srt_path = str(srt_path)
            job.status = 'finished'
            job.message = '完成'

            # 清除检查点
            self.checkpoint_service.delete(job.job_id)

    def _transcribe_all_segments(
        self,
        job: JobState,
        segments: List[Dict],
        skip_indices: set = None,
        existing_results: List[Dict] = None
    ):
        """
        转录所有音频段（支持断点续传）

        Args:
            job: 任务状态
            segments: 所有段信息
            skip_indices: 要跳过的段索引（已处理）
            existing_results: 已有的转录结果
        """
        skip_indices = skip_indices or set()
        results = existing_results or []
        processed_indices = list(skip_indices)

        self._update_progress(job, 'transcribe', 0, '加载模型中')
        model = self._get_model(job.settings)
        align_cache = {}

        for idx, seg in enumerate(segments):
            # 跳过已处理的段
            if idx in skip_indices:
                self.logger.debug(f"⏭️ 跳过已处理段: {idx}/{len(segments)}")
                continue

            if job.canceled:
                raise RuntimeError('任务已取消')

            # 更新进度
            ratio = (idx + 1) / len(segments)
            self._update_progress(
                job,
                'transcribe',
                ratio,
                f'转录 {idx+1}/{len(segments)}'
            )

            # 转录当前段
            seg_result = self._transcribe_segment(seg, model, job, align_cache)
            if seg_result:
                results.append(seg_result)
                processed_indices.append(idx)

            job.processed = len(processed_indices)

            # 每5段保存一次检查点
            if (idx + 1) % 5 == 0:
                progress = 15 + ratio * 80
                self.checkpoint_service.save(Checkpoint(
                    job_id=job.job_id,
                    phase='transcribe',
                    progress=progress,
                    segments=[
                        {"file": str(s['file']), "start_ms": s['start_ms']}
                        for s in segments
                    ],
                    total_segments=len(segments),
                    processed_segments=processed_indices,
                    results=results,
                    language=job.language
                ))
                self.logger.debug(f"💾 检查点已更新: {idx+1}/{len(segments)} 段")

        # 所有段转录完成
        self._update_progress(job, 'transcribe', 1, '转录完成 生成字幕中')

        # 保存完成状态的检查点
        self.checkpoint_service.save(Checkpoint(
            job_id=job.job_id,
            phase='srt',
            progress=95.0,
            results=results,
            language=job.language
        ))

        # 生成SRT
        base_name = os.path.splitext(job.filename)[0]
        srt_path = Path(job.dir) / f'{base_name}.srt'

        self._generate_srt(results, str(srt_path), job.settings.word_timestamps)

        job.srt_path = str(srt_path)
        job.status = 'finished'
        job.message = '完成'

        # 清除检查点
        self.checkpoint_service.delete(job.job_id)
        self.logger.info(f"✅ 任务完成: {job.job_id}")
```

---

#### 步骤3.4: 添加恢复API (30分钟)

```python
# backend/app/main.py

@app.post("/api/resume/{job_id}")
async def resume_transcription(job_id: str):
    """
    恢复中断的转录任务

    Args:
        job_id: 任务ID

    Returns:
        dict: 恢复结果
    """
    try:
        # 检查任务是否存在
        job = transcription_service.get_job(job_id)
        if not job:
            raise HTTPException(status_code=404, detail="任务不存在")

        # 检查是否有检查点
        if not transcription_service.checkpoint_service.exists(job_id):
            raise HTTPException(status_code=404, detail="没有找到可恢复的检查点")

        # 检查任务状态
        if job.status == "processing":
            raise HTTPException(status_code=400, detail="任务正在运行中")

        # 重新启动任务（会自动从检查点恢复）
        transcription_service.start_job(job_id)

        return {
            "success": True,
            "message": "任务已从检查点恢复",
            "job_id": job_id
        }

    except HTTPException:
        raise
    except Exception as e:
        logger.exception(f"恢复任务失败: {job_id}")
        raise HTTPException(status_code=500, detail=str(e))
```

---

#### 步骤3.5: 测试断点续传 (1小时)

**手动测试步骤**:

1. **开始一个转录任务**
```bash
# 启动服务
python backend/app/main.py

# 上传一个较长的视频（5-10分钟）
# 等待转录开始
```

2. **中断任务**
```bash
# 在转录进行到一半时，关闭服务（Ctrl+C）
```

3. **重新启动服务**
```bash
# 再次启动
python backend/app/main.py
```

4. **恢复任务**
```bash
# 调用恢复API
curl -X POST http://localhost:8000/api/resume/{job_id}

# 或者在前端点击"恢复"按钮
```

5. **验证结果**
- 检查日志，确认从检查点恢复
- 确认不会重新处理已完成的段
- 最终生成的字幕文件完整

**提交变更**:
```bash
git add .
git commit -m "feat: 实现断点续传功能"
```

**完成标准**:
- ✅ 检查点服务已实现
- ✅ 转录服务支持断点续传
- ✅ 每5段保存一次检查点
- ✅ API支持恢复任务
- ✅ 手动测试通过
- ✅ 恢复后生成的字幕完整

---

### 阶段4: 收尾优化 (Day 4)

#### 步骤4.1: 前端进度显示优化 (可选, 2小时)

**如果时间充足，可以实现SSE替代轮询**

```python
# backend/app/main.py

from sse_starlette.sse import EventSourceResponse
import asyncio

@app.get("/api/status/{job_id}/stream")
async def stream_job_status(job_id: str):
    """
    使用SSE流式传输任务状态

    Args:
        job_id: 任务ID

    Returns:
        EventSourceResponse: SSE响应
    """
    async def event_generator():
        """生成SSE事件"""
        while True:
            job = transcription_service.get_job(job_id)
            if not job:
                yield {
                    "event": "error",
                    "data": json.dumps({"error": "任务不存在"})
                }
                break

            # 发送状态
            yield {
                "event": "status",
                "data": json.dumps(job.to_dict())
            }

            # 任务完成或失败，停止流
            if job.status in ["finished", "failed", "canceled"]:
                yield {
                    "event": "complete",
                    "data": json.dumps(job.to_dict())
                }
                break

            # 等待1秒后再次检查
            await asyncio.sleep(1)

    return EventSourceResponse(event_generator())
```

**前端使用SSE**:
```javascript
// frontend/src/App.vue

function watchJobProgress(jobId) {
  const eventSource = new EventSource(`/api/status/${jobId}/stream`)

  eventSource.addEventListener('status', (event) => {
    const data = JSON.parse(event.data)
    // 更新进度显示
    updateProgress(data)
  })

  eventSource.addEventListener('complete', (event) => {
    const data = JSON.parse(event.data)
    // 任务完成
    handleComplete(data)
    eventSource.close()
  })

  eventSource.addEventListener('error', () => {
    eventSource.close()
  })
}
```

---

#### 步骤4.2: 打包预检查 (1小时)

**检查清单**:

1. **路径检查**
```bash
# 搜索硬编码路径
grep -r "C:\\" backend/
grep -r "D:\\" backend/
grep -r "/Users/" backend/
grep -r "/home/" backend/

# 应该没有结果
```

2. **FFmpeg检查**
```python
# 确认config.py中的FFmpeg逻辑
from core.config import config

ffmpeg_cmd = config.get_ffmpeg_command()
print(f"FFmpeg命令: {ffmpeg_cmd}")

# 测试是否可用
import subprocess
result = subprocess.run([ffmpeg_cmd, "-version"], capture_output=True)
print(result.returncode == 0)
```

3. **模型路径检查**
```python
# 确认环境变量已设置
import os
print(f"HF_HOME: {os.environ.get('HF_HOME')}")
print(f"TORCH_HOME: {os.environ.get('TORCH_HOME')}")

# 应该都指向项目目录
```

4. **依赖导出**
```bash
# 导出Python依赖
pip freeze > requirements.txt

# 或者使用conda
conda env export > environment.yml
```

---

#### 步骤4.3: 更新文档 (1小时)

**更新README.md**:

```markdown
# Video-to-SRT GPU

视频转字幕工具，支持GPU加速和断点续传

## ✨ 新功能

- ✅ **断点续传**: 任务中断后可以继续，不会丢失进度
- ✅ **独立打包**: 所有路径使用相对路径，支持便携部署
- ✅ **智能分段**: 基于静音检测，避免句子断裂

## 🚀 快速开始

### 1. 安装依赖

```bash
# 使用conda (推荐)
conda create -n video_to_srt python=3.10
conda activate video_to_srt
pip install -r requirements.txt
```

### 2. 准备FFmpeg

下载FFmpeg并解压到项目根目录:
```
项目根目录/
└── ffmpeg/
    └── bin/
        └── ffmpeg.exe
```

### 3. 启动服务

```bash
# 方式1: 使用启动器（推荐）
python simple_launcher.py

# 方式2: 手动启动
python backend/app/main.py
```

### 4. 访问界面

打开浏览器访问: http://localhost:5174

## 📖 使用说明

### 基本流程

1. 上传视频文件
2. 配置转录参数（模型、设备等）
3. 开始转录
4. 等待完成
5. 下载字幕

### 断点续传

如果转录过程中断：

1. 重新启动服务
2. 在任务列表中找到中断的任务
3. 点击"恢复"按钮
4. 任务会从中断处继续

## 🔧 配置说明

所有配置在 `backend/app/core/config.py` 中:

- `DEFAULT_MODEL`: 默认模型 (tiny/base/small/medium/large)
- `SEGMENT_LENGTH_MS`: 分段长度 (毫秒)
- `MAX_CACHE_SIZE`: 最大模型缓存数量

## 📦 独立打包

项目已支持独立打包部署：

- ✅ 所有路径使用相对路径
- ✅ FFmpeg集成到项目内
- ✅ 模型缓存在项目目录

## 📝 许可证

MIT License
```

---

## 四、风险控制

### 4.1 给独立开发者的建议

1. **不要过度抽象**
   - 不需要写 `BaseService` 这种基类
   - Python鸭子类型就够了
   - 只在确实需要时才抽象

2. **日志优于Debug**
   - 在关键步骤加 `logger.info()`
   - 出问题时看日志比断点调试快
   - 日志要有emoji，方便扫视

3. **Commit粒度**
   - 删文件前 Commit 一次
   - 搬运代码后 Commit 一次
   - 黄金测试通过后 Commit 一次
   - **千万不要攒到最后一起提交**

4. **及时备份**
   - 每完成一个阶段创建tag
   - 重要改动前先创建分支
   - 可以随时回滚

### 4.2 回滚策略

如果重构出现严重问题:
```bash
# 查看所有备份标签
git tag -l "backup-*"

# 回滚到重构前
git reset --hard backup-before-refactor-20251117

# 或切换回主分支
git checkout feature/breakpoint-resume
```

---

## 五、验收标准

当你完成上述步骤，你应该达到：

### 5.1 功能验收

- ✅ 项目只有一个 `main.py`，点开就能跑
- ✅ 关掉运行窗口，再重新打开，未完成的任务能接着跑（断点续传）
- ✅ 运行"黄金标准测试"，生成的字幕和重构前一致
- ✅ 代码里没有硬编码的绝对路径

### 5.2 代码质量

- ✅ 代码重复率 < 5%
- ✅ 单个文件不超过500行
- ✅ 所有配置集中在config.py
- ✅ 日志信息完整清晰

### 5.3 独立打包

- ✅ FFmpeg使用项目内的版本
- ✅ 模型下载到项目目录
- ✅ 所有路径使用相对路径
- ✅ 环境变量不影响运行

---

## 附录

### A. 测试检查清单

每完成一个阶段，都要运行这个检查清单：

```bash
# 1. 启动测试
python backend/app/main.py
# ✅ 能正常启动

# 2. 上传测试
# 上传 tests/fixtures/test_video.mp4
# ✅ 上传成功

# 3. 转录测试
# 开始转录
# ✅ 转录完成

# 4. 黄金标准测试
python tests/test_golden_master.py
# ✅ 测试通过

# 5. 断点续传测试（阶段3后）
# 开始转录 -> 中断 -> 重启 -> 恢复
# ✅ 能从断点继续
```

### B. 常见问题

**Q1: 黄金标准测试失败怎么办？**

A: 检查差异：
```bash
# 对比文件
diff tests/fixtures/golden_standard.srt output/new.srt

# 如果只是时间戳微小差异（<100ms），可以接受
# 如果文本内容不一致，说明重构改坏了
```

**Q2: 检查点保存失败？**

A: 检查目录权限：
```bash
# 确保jobs目录可写
ls -la jobs/

# 检查检查点文件
cat jobs/{job_id}/checkpoint.json
```

**Q3: FFmpeg找不到？**

A: 检查路径配置：
```python
from core.config import config
print(config.get_ffmpeg_command())
print(config.FFMPEG_EXE.exists())
```

---

**文档结束**

*这份计划是一个实战指南，边做边调整。祝重构顺利！* 🚀
