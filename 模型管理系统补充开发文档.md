...
sequenceDiagram
    ...
    loop 每秒推送进度
        ...
        Frontend->>Frontend: 更新进度条 35%
    end

    HF-->>Service: 下载完成
    Service->>SSE: 推送完成事件
    SSE-->>Frontend: event: model_complete
    Frontend->>Frontend: 显示"下载完成"通知
    Frontend->>Frontend: 更新状态为 "ready"
```
